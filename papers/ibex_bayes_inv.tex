...
Real data brings some additional complications when comparing to computer models
output. Scientists know that the makeup of the heliosphere changes over time,
largely due to the variations in the solar wind that accompany the sun's 11 or
21-year cycle. Physicists therefore build in this evolution to their simulators
either through additional parameters or restricting the application of their
models to a specific timeframe. This latter constraint is the case for the GDF
model, which corresponds to 2009-2011, the first few years of IBEX's mission. In
another approach, the ribbon model is averaged over the entire cycle of the sun.
We factor in these heliospheric variations by only comparing satellite data from
...
Our experimental setup will be similar to that in Figure \ref{f:ibex_synth_ex},
...
from Section \ref{sec:surr_skymaps} is that field data ($Y^F, X^F$) are not
...
$z$ from years 2009-2011. Additionally, we only use ENAs collected at energy
level ESA 4. ESA 4 allows the best context for analysis, whereas lower ESAs
have a much weaker signal and higher ESAs overwhelm the analysis. Raw satellite
data from 2009-2011 and our results are shown in Figure \ref{f:ibex_real_ex}.
...
%------------------------------------------------------------------------------
%% SECTION 5.3 - COMPUTER MODEL INSUFFICIENCIES
\subsection{Computer Model Insufficiencies}
\label{sec:ibex_discrep}
%------------------------------------------------------------------------------

% \begin{figure}[ht!]
% \centering
% \includegraphics[scale=0.44,trim=0 15 70 0,clip=TRUE]{ibex_field_disc_09.pdf}
% \includegraphics[scale=0.44,trim=50 15 70 0,clip=TRUE]{ibex_field_disc_10.pdf}
% \includegraphics[scale=0.44,trim=50 15 0 0,clip=TRUE]{ibex_field_disc_11.pdf}
...
satellite. It's clear that the computer model does not sufficiently account for
the behavior observed in the satellite data. But this is not terribly
surprising. Simulators are inherently biased, simply from not being able to
completely reconstruct the real process. \citet{kennedyohagan2001} address this
limitation by introducing a discrepancy term in their canonical computer model
calibration framework, a practice followed by other practitioners
...
competing interests between different regimes in the sky map. Future work is
needed to address this potential nonstationarity in the discrepancy surface.
...
expensive, limiting the number of runs available for analysis, demanding a GP
surrogate, but 2) the computer model output is extremely high-dimensional,
requiring unconventional methods induce sparsity and approximate the GP, and 3)
data from a field experiment are Poisson counts, bucking the current Bayesian
inverse problem literature that focuses on normally distributed field
observations. Our proposed Poisson Bayesian inverse problem framework is able to
recover the model parameter $u^*$ at the nominal 95\% rate when provided with
synthetic data where the truth is known. We applied this methodology to data
collected from the IBEX satellite and corresponding simulated heliospheric sky
maps from a computer model. When faced with data collected from the satellite,
our method shows that complex discrepancies exist between the simulator and the
physical process it attempts to represent. This bias is unable to be resolved
with a simple multiplicative constant. More advancements are needed to properly
model this disparity. Further collaboration with theoretical astrophysicists
responsible for computer model development could also be pursued to improve the
simulator's fidelity to the truth.

Our framework is only applied to counts assumed to come from a Poisson
distributed process. We encourage practitioners to use this work in other
contexts where field observations must be modeled differently (e.g. Negative
Binomial, Bernoulli, Exponential, etc.). Additionally, the Scaled Vecchia GP
surrogate used for the computer model, although extremely accurate, fails to
provide full uncertainty quantification for it's parameter estimates. A fully
Bayesian framework could be developed, perhaps with the use of a deep Gaussian
process, to handle the inherent nonstationarity in the discrepancy.

...
seeking to gain a more in-depth understanding. The goal is often one of two
...
how the process was initiated, but know the value of model parameters,
...
simple, but classical, approach to this problem is to find the best match
between observed data and computer model output via some minimization of error
...
of Figure \ref{f:inv_v_classic}. But in Bayesian Inverse Problems, the
elements of the vector $\boldsymbol \theta$ correspond to inputs for a
physical or mathematical model or process. As depicted in Figure
\ref{f:inv_v_classic}, parameters $\boldsymbol \theta$ are passed in to the
model, blackbox or not, which in turn generates output dependent on the
values of the input. That model response forms the statistical parameters
underlying the data-generating process of $Y$. In classic Bayes,
$\boldsymbol \theta$ is used to evaluate the likelihood $L(Y|\boldsymbol
\theta, X)$. But in Bayesian Inverse Problems, $\boldsymbol
\theta$ is only pertinent to a simulation or model, and not a direct part of
the likelihood calcuation. Instead, the simulator or model output is used to
...
through $\pi(\boldsymbol \theta)$ over model parameters, or rather use an
...
approach in most of the literature. If $\pi(\boldsymbol
\theta)$ is conjugate, inference for $\boldsymbol \theta$ is simple and can be
done analytically. However, this is often not the case, requiring an
approximation of the posterior (i.e.~Laplace, Variational Bayes). Markov chain
Monte Carlo (MCMC) methods can also be employed to sample from the posterior,
...
of different settings of parameters $\boldsymbol{\theta} = \begin{pmatrix} \mu

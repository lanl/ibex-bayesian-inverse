...
{\color{red}(for our motivating application, $n_F > 23,000$, $p_F=3$, and $p_U
= 2$)}. These {\color{red}$X$ locations} may be the locations {\color{red}
$X^F \in \mathbb{R}^{n_F \times p_F}$ } that correspond to field measurements
$Y^F$, or a grid of predictive locations supporting high-resolution visuals.
{\color{red}Therefore, computer model and field data are potentially collected
at different inputs, but we only evaluate the likelihood
(Eq.~\ref{eq:inv_bayes_mod}) at observed field data locations.} Given prior
...
this setting, generally with Gaussian field data $Y^F$ {\color{red}following
an $n_F$-variate normal with defined mean and covariance functions.}
\begin{align}
...
over a fine grid $X$) are displayed in the left column. On the top, red stars
are measurements with Gaussian noise. On the bottom, red stars are individual
...
Y_{2010}^{F^T} \quad Y_{2011}^{F^T}]^\top\right)$. Although the data come from
...
calculation, $\mathcal{L}(Y^F|\lambda(X^F)^{(t)}, e(X^F)) = \prod_{i=1}^{n_F}
f(y_i|\lambda_i(X^F)^{(t)}, e_i)$. The IBEX simulator itself does not embody
...
developed to provide full uncertainty quantification, perhaps with the use of
...
model runs increases. One intriguing extension could be the combination of
\citeauthor{higdoncalib2008}'s PCA approach and the Vecchia approximation. PCA
weights could be modeled using the Vecchia approximation and alleviate a
growing number of computer model runs. To our knowledge this has not been
...
particular as it relates to computer model calibration. It has been used in
multiple academic works \citep{chan2024constructing,surer2022uncertainty}.
...
{\color{red} Figure \ref{f:ibex_sim_surr} helps visualize the accuracy of our
proposed method (SVEC) by displaying a few computer model evaluations
side-by-side with predicted surfaces at unobserved parameter settings. Figure
\ref{f:ibex_surr_metrics} then presents performance metrics from a
hold-one-out bakeoff between SVEC and multiple competitors. At each iteration,
all methods are trained on 65 model runs of the IBEX simulator and then asked
to predict the response at the held out model parameter. Predictions are then
used to calculate the displayed RMSE and CRPS values. To further visually
demonstrate the differences between competing methods, we plot the residuals
(averaged over the 66 held out model runs) from each method's predicted
surfaces. These are shown as heat maps in Figure \ref{f:surrogate_residuals}.
}
...
the residuals and then take their square root. Additionally, the range of
...
Otherwise, heat maps for SVEC and SEPIA would be solid red and heat maps for
{\sf laGP} and {\sf deepgp} would be solid white. }
...
varying values of neighborhood size $m$ (for SVEC) and number of principal
components $n_k$ (for SEPIA). It is noteworthy that {\sf laGP} performs worse
(and is highly variable) than {\sf deepgp} in terms of L-infinity distance. We
believe this could be due to the unique framework of {\sf laGP}, where each
prediction is based on a unique neighborhood of surrounding points, and
therefore largely independent of each other.}

...
data) and a computer implementation of a physical model $m(\cdot)$ with
...
\begin{figure}[ht!]
\centering
\includegraphics[scale=0.7,trim=0 0 0 0,clip=TRUE]{inv_v_classic.pdf}
\caption{An illustration of classic Bayesian statistics (top)
and the approach to Bayesian Inverse Problems (bottom). Output $Y^F$ is Gaussian
in either case, but parameters $\boldsymbol{\theta}$ differ in where they
originate.
\label{f:inv_v_classic}}
\end{figure}

We start with a very simple example that we will return to in Section
\ref{sec:gen_bayes_inv}. Consider the left panel of Figure \ref{f:toy_calib}.
Red stars denote field measurements ($Y^F$) of some true, unknown process at
different locations $X^F$, observed with Gaussian noise. Additionally, multiple
observations are made at each location, providing replication. This is common,
due to the expense of changing configurations of a physical experiment, but it
need not be the case. Light gray lines indicate individual runs of a computer
simulation $m(\cdot)$ at a sample of different settings of parameters
$\boldsymbol{u} = \begin{bmatrix} u_1 & u_2
\end{bmatrix}^T$. The black, bold, dashed line shows the underlying process
$m(\cdot)$ when $\boldsymbol{u}$ is set to the truth.
\begin{figure}[ht!]
\centering
...
computer model $m(\boldsymbol{u}^*)$ to get $\boldsymbol{\theta^*}$. Then, we
evaluate the MH-ratio and either accept or reject the proposed values. Predictions
and posterior estimates for our example are shown in the middle and right panels of
Figure \ref{f:toy_calib}. Note that each of the light gray lines in the middle panel
represent a run of the simulation at one of the accepted set of model parameters,
$\boldsymbol{u}^{(t)}$. That is, each gray line corresponds to a circle in the right
panel, where posterior draws of $\boldsymbol{u}$ are displayed. It is evident that
the MH sampler thoroughly explores the input space and that it successfully captures
the true value of $\boldsymbol{u}$.
...
assuming $Y^F \sim \mathcal{N}\!\left(\mu(\boldsymbol{u}),
\Sigma(\boldsymbol{u})\right)$ as a likelihood, where
$\boldsymbol{\theta}(\boldsymbol{u})=(\mu(\boldsymbol{u}),
\Sigma(\boldsymbol{u}))$ depends on $m(\boldsymbol{u})$. Yet our IBEX
...
The biggest hurdle that we did not address in our toy example is the expense of the
computer model. Even with modern-day computing power, some simulations can take
hours, days, or weeks to complete. So, it's not practical to run $m(\cdot)$ inside
the MH sampler. This constrains the amount of runs that can be executed and therefore
restricts the exploration of the model parameter space. Therefore, we need a model
(what we call a surrogate, or emulator) fit to the limited simulator output to make
accurate and reliable predictions for out-of-sample values of our computer model
parameters. Any statistical model can serve as a surrogate, ranging from linear
...
distributed with some mean $\mu(X)$ and covariance $\Sigma(X)$, clearly both
functions of $X$. Then, predicting at a new location $x$ is relatively
straightforward. Eq.~(\ref{eq:mvn_eq}) articulates how to obtain mean
predictions $\mu^*(x)$ and corresponding uncertainty quantification estimates
$\Sigma^*(x)$.
\vspace{-0.75cm}
...
motivating the desire for a model that interpolates. GPs are then useful by providing
...
estimating the parameters of a physical process from which they could gather
only a small set of observations, but that could be represented by a
simulation. But these computer models were expensive, producing limited runs
with extremely high-dimensional output in the form of images, shape
description, time series, etc. So, in order to efficiently fit a surrogate
model to the simulator, they needed to perform an approximation to decrease
the size of the data. As a solution, they employed principal components
...
sum of $n_k$ basis functions $\boldsymbol{k}_1, \boldsymbol{k}_2, \ldots ,
\boldsymbol{k}_{n_k}$, each scaled by a corresponding weight $w_i$, as shown
in
\begin{align}
...
is the vector of model parameters they were interested in estimating.
...
when scaling to large $n$ and can be completed in a reasonable amount of time.
Instead of modeling the high-dimensional response $\boldsymbol y$, the GP only
modeled the small set of component weights. Bayesian inference was used to
obtain the posterior distribution of these component weights. Then, combining
estimates of the weights with the calculated basis functions allowed for
predictions out of sample. As a result of this approximation, the dimension of
the covariance matrices $\Sigma(X)$ and $\Sigma(X^*)$ in Eq.~\ref{eq:mvn_eq}
remained small and was able to be decomposed quickly.

...
Equation \ref{eq:mod_with_disc} includes a simple, multiplicative discrepeancy
term $\delta$. We tested using this model on the IBEX satellite data and
discovered that it was insufficient to account for the spatially-varying,
nonstationary discrepancy present in the IBEX data. To verify that our scale
discrepancy actually works, we tested it on simulated data. For ten different
scales between 0 and 10, we generated data.

Figure \ref{f:sim_pit_hist} shows posterior samples for 10 runs of Algorithm \ref{alg:gibbs}, where $Y^F$ is drawn from varying scaled ENA rates.
...
scaled by separate known values ($\delta_{true}$). True values marked in blue.
Posterior mean and 95\% credible intervals shown in red.
\label{f:sim_pit_hist}}
\end{figure}

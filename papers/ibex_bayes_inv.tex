...
\ref{eq:inv_bayes_mod} provides the typical hierarchical model used in this
setting.
\begin{align}
...
\end{align}
One big difference with Bayesian inverse problems, as opposed ordinary
...
interested in settings $u$, dialing a setting in thesimulation, that define
...
which $u$ are likely to have generated the observed $Y^F$ values in the left
...
\mathcal{N}(m(u^\star, x_i), \sigma^2)$ for a ``true'' unkonwn $u^\star =
(u_1^\star, u_2^\star)^\top$ and uniform $x_i \in X_F$.  In this simple
...
different $u$ on an fine grid of spatial locations $X$. The bold-black-dashed
...
normal likelihood whose mean $\mu(X_F)=m(\cdot)$ and whose covariance $\Sigma(X_F) \sim \sigma^2
...
high-resolution grid $X$, the sampler only uses evaluations of $m(u, X^F)$ in
...
Figure \ref{f:fig1}), whose observations are counts under variable exposure.
Our first contribution, which is albeit rather straightforward, is to depart
from that literature and Eq.~\ref{eq:inv_bayes_mod} by perusing a Poisson
likelihood: $Y^F \sim \mathrm{Pois}(\lambda(X^F), e(X_F))$, where
$\lambda(X^F)$ is determined by $m(u, X_F)$ using exposures $e(X_F)$ provided
with $Y_F$. Details are provided soon in Section \ref{sec:gen_bayes_inv}.

Another challenge to the typical apparatus involves computation.  Each run of
the IBEX model $m(\cdot, \cdot)$ is a Herculean effort, requiring thousands of
core hours on a supercomputer.  That simply can't be embedded in a Metropolis
sampler requiring thousands of iterations to convergence.  While it's rather
...
effectively is challenged by the dimension of $X_F$.  Herein lies our main
...
this section outlines the standard surrogate modeling toolkit, as a
jumping-off point toward explaining both our main comparative (Section
\ref{sec:pca}) and our main methodological contribution.

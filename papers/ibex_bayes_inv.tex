...
A Gaussian process \citep[e.g.,][]{rasmussen2003gaussian} is a realization of a
multivariate normal (MVN) distribution, governed by some mean and covariance
function. GPs are fit to observed data and used to make predictions at new input
locations, relying on the conditional multivariate normal or \textit{kriging}
equations \citep{matheron1963principles}. For example, suppose we have a vector
of observations $Y^M$ at input locations $X^M$. Here we use the superscript $M$
to make explicit that our GP surrogate is modeling the simulator input and
output, not field data. In our situation, the expensive computer model
$m(\cdot,\cdot)$ is run over a small design of space-filling $u$-values ($U$) on
a dense grid of spatial locations $X$. That is, $X^M = \begin{bmatrix} 1_{n_U}
\otimes X & U \otimes 1_{n_X} \end{bmatrix}$. Two such runs are shown in Figure
\ref{f:fig1}. But to evaluate the likelihood (for $Y_F$) in our Metroplis
sampler, we need fast and reliable predictions at proposed (and so far
unobserved) values of our model parameters, $u^*$, and at field locations $X^F$.
In other words, $\hat{\mu}(X^F)$ from Eq. \ref{eq:inv_bayes_mod}.

Suppose $Y^M$ is MVN with mean $\mu\!\left(X^M\right)$ and covariance
$\Sigma\!\left(X^M\right)$. Then, predicting at a new location $x$ (think
proposed $u^*$ and point on grid $X^F$) follows
\begin{align}
Y^M(X^M) &\sim \mathcal{N}_{n_M}(\mu(X^M), \Sigma(X^M)) &
\hat{\mu}(x) &= \mu(x) + \Sigma(x, X^M) \Sigma(X^M)^{-1} (Y^M - \mu(X^M)) \nonumber \\
Y(x) \mid Y(X^M) &\sim \mathcal{N}(\hat{\mu}(x), \hat{\sigma}^2(x)) &
...
common to set $\mu(X^M)=0$ assuming pre-centered responses. In this zero-mean
...
\citep{stein1999interpolation} in our work, of which both the Gaussian and
Exponential kernels are special cases parameterized by a nugget $g$,
lengthscales $\theta$, and scale $\tau^2$, whose sampling is included along with
others in posterior integration via MCMC. GPs provide a flexible, nonparametric
regression tool. In addition to enjoying canonical status as surrogates for
computer simulation experiments, they are utilized in geospatial statistics
...
% The reason for their popularity as surrogate
% models for computer experiments is their ability to interpolate
...
% are useful by providing predictive surfaces that give higher estimates of
% variance away from observed data, and virtually no variance at observed
% locations. Perhaps the hallmark of GPs is that they naturally provide
...
% are widely used in Bayesian inverse problems.
...
a Bayesian MCMC setting.  In our IBEX setting, the output ``images'' are
massive ($\gg 15{,}000$ pixels) for each $u$-value.  Most of our modeling
effort involves circumventing this bottleneck, however there is a method
tailor-made for this setting when the likelihood (for $Y_F$) is Gaussian.

...
observation) and a computer implementation of a physical model $m(u, X)$, where
parameters $u$ are unknown and $X$ represents a dense grid at which to evaluate the
model. Given prior information about $u$, we want to know which $u$'s lead to
realizations of $m(\cdot)$ that could explain $Y^F$, i.e., the posterior for $u \mid
Y^F$. This is a classic Bayesian inverse problem
\citep{kaipio2011bayesian,stuart2010inverse,knapik2011bayesian}. Equation
\ref{eq:inv_bayes_mod} provides a hierarchical model, contrasting classical Bayesian
inference with our computer model setting.
\begin{align}
...
statistical modeling, is that the parameters $\mu\left(X^F\right)$ and
$\Sigma\left(X^F\right)$ defining the distribution of $Y^F$ are not of direct interest.
Rather, we are interested in the settings $u$ that define
$\mu(X)$ and $\Sigma(X)$ through $m(u,
X)$, which depends on $u$ and whatever spatial field, or grid,
$X$ determines it's values.  For example, two different settings of
$u$ were used to generate the center and right panels of Figure
\ref{f:fig1}. Those colors indicate output $\mu(X)$ values on a dense grid
$X$ of longitude and latitude. We are interested in learning about which
$u$ are likely to have generated the observed $Y^F$ values in the left
panel at locations $X=X^F$.

To help fix ideas, consider the simpler example introduced in the left panel of
Figure \ref{f:toy_calib}. Red stars indicate noisy field measurements,
$Y^F$, observed in replicate under some true, unknown process at
different field locations $X^F$. A computer simulation
$m(u, X)$ was built to virtualize that process under a
physical model with two unknown parameters $u = (u_1,  u_2)^\top$. Each
light gray line is a realization of $m(\cdot)$ for a different $u$ on a
constant, fine grid $X$. The bold-black-dashed line is $m(\cdot)$ at the
(presumed unknown) truth.
...
normal likelihood whose mean is $m(\cdot)$ and whose covariance is $\sigma^2
\mathbb{I}$, where $\sigma^2$ is marginalized out under a reference prior. These are
not of direct interest in and of themselves.  Rather, the main object of interest is
summarized in the right panel: the posterior samples of $u$. It is
clearly evident that the Metropolis sampler thoroughly explores the input space and
that it successfully captures the true value of $u$. We aim to provide a
method that can achieves the same goal, but for the parameters $u$ that
underlie the IBEX satellite data (left panel of Figure \ref{f:fig1}).
...
%
Yet our IBEX observations are counts. Our first contribution, which is albeit rather
straightforward, is to depart from that literature by perusing a Poisson likelihood:
$Y^F \sim Poisson(\lambda(X^F))$, where $\lambda(X^F)$ is determined by $m(u, X)$. We
...
requiring thousands of core hours on a supercomputer.  That simply can't be embedded
in a Metropolis sampler requiring thousands of iteration to convergence.  While it is
rather conventional to deploy a surrogate model \citep[e.g.][]{gramacy2020surrogates}
in this setting, trained on a design of space-filling $u$-values, doing
so effectively is challenged by the dense grid of output, over longitude and latitude
via $X$ for each $u$.  Herein lies our main methodological
contribution. The remainder of this section outlines the conventional surrogate
modeling toolkit, as a jumping-off point toward explaining both our main comparative
(Section \ref{sec:pca}) and our main methodological contribution.
...
$m(u)$ that is run. Next, we review what has been done for applications
...
sum of $n_k$ basis functions $k_1, k_2, \ldots , k_{n_k}$, each scaled by a
corresponding weight $w_i$. They specify each model response
\begin{align}
y(x) &\approx \sum_{i=1}^{n_k}  k_i \cdot w_i (x), \quad n_k \ll n \ \nonumber
\\ w_i(x) &\sim \mathcal{N}( 0, \Sigma_{w_i}),
\label{eq:higdon_pca}
\end{align}
where $x$ is a single row of the matrix $X^M$. Note that $X^M$ in this approach is
small, only containing entries corresponding to the unique values of $x = u$ executed
by the computer model. Weights $w_i$ are realizations of a GP, fit over principal
components from the limited simulator output. Therefore, by not directly modeling the
high-dimensional respone $y$, the decomposition of $\Sigma_{w_i}$ avoids the cubic
bottleneck in computation when scaling to large $n$ and can be completed in a
reasonable amount of time. Once the surrogate is fit, Bayesian inference via MCMC is
used to obtain the posterior distribution of these component weights. Then, combining
estimates of the weights with the calculated basis functions allow for out-of-sample
predictions. As a result of this approximation, the dimension of the covariance
matrices $\Sigma(X^M)$ and $\Sigma^*(x)$ in Eq.~\ref{eq:mvn_eq} remains small and is
able to be decomposed quickly.

We propose a simpler approach in Section \ref{sec:vecchia}, devoid of the definition
and selection of how many basis functions to use. Although this basis representation
was and still is successful in many applications, we argue that what
\citet{higdoncalib2008} really wanted to do was represent the functional output as a
univariate response. That is, replicate the entry in the input matrix $X^M$ for each
response the number of times equal to the length of the output vector. In essence,
implicitly encode the index (e.g.~pixel in image, moment in time series) into the
response vector for each observation in the input matrix. We believe that had
\citet{higdoncalib2008} had access to the capacity of modern-day computing, along
...
way. We will show that our more straightforward approach results in equal or improved
performance in both predictive accuracy and uncertainty quantification, along with a
significant reduction in computational expense. Additionally, their method and
implementation, originally written in {\tt MATLAB} and now deployed in {\tt Python}
\citep{gattiker2020lanl}, is unable to model a non-Gaussian field response, leading
to our first contribution.

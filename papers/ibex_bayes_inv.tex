...
allowing for broad use in a variety of different fields where GPs are employed,
particularly spatial statistics and computer experiments
\citep{katzfuss2020vecchia,katzfuss2021general,zhang2022multi,sauer2023vecchia}.
\begin{align}
\mathcal{L}(Y^M)&=\prod_{i=1}^n \mathcal{L}(Y^M_i \mid Y^M_{g(i)})
\hspace{0.3em} \mbox{ where } g(1)=\emptyset \hspace{0.3em} \mbox{ and }
g(i)=\{1, 2, \ldots, i-1\} \nonumber \\
&\approx \prod_{i=1}^{n}\mathcal{L}(Y^M_i \mid Y^M_{h(i)}) \hspace{0.3em} \mbox{
...
approach for selecting the conditioning set, taking a page from previous
...
their paper for a more in-depth explanation of their work. But the main crux of
this implementation is the pre-scaling of inputs by dimension \textit{before}
the creation of conditioning sets based on nearest neighbors.
\citeauthor{scaledvecchiakatzfuss2022} point out that a response can change at
varying rates by dimension, and therefore predictions should be made based on
neighborhoods that account for this heterogeneity in each dimension's impact.
Scaling the data beforehand factors in these dynamics. In fact, this is why
Figure \ref{f:ibex_vecchia} shows conditioning sets tightly clustered in
latitude and longitude, but broadly dispersed in $u$. Internally, Scaled Vecchia
recognizes that the response surface changes more rapidly in grid space than in
the model parameters. Therefore, a reference point shares much in common with
observations far away in model parameter space, but needs information about the
nature of the response surface from latitudes and longitudes in close proximity.
This simple yet innovative idea has propelled Scaled Vecchia to prominence in
the computer experiments community. Publicly available code for
\citeauthor{scaledvecchiakatzfuss2022}'s implementation that we directly rely on
is readily accessible at \url{https://github.com/katzfuss-group/scaledVecchia}.
...
Next we assess Scaled Vecchia's performance on the IBEX simulator, first with a
visual example. Scientists at LANL have provided us with 66 unique model runs,
each producing an image of 16,200 points or pixels. Figure \ref{f:ibex_sim_surr}
...
parameters that fall in between the actual runs shown. These are depicted in the
cross of Figure \ref{f:ibex_sim_surr}, those panels with thicker, dashed
...
surfaces change gradually and as one would expect. But we are not satisfied with
simply passing the eye test, so we turn to empirical results and show that our
surrogate fit is extremely accurate and outperforms other current methods.
...
predict at the held out combination of model parameters. We then evaluate how
...
\citep{gattiker2020lanl}, implemented in {\sf Python}. Figure
...
SEPIA and $m=25$ for Scaled Vecchia. We'll come back to the others in a moment.
It is obvious that the Scaled Vecchia approximation outperforms {\sf laGP} and
{\sf deepgp} in both RMSE and CRPS. We gave {\sf laGP} the default neighborhood
size of 50, but the predictions are made based only on observations local
(without pre-scaling the inputs) to the predictive location, perhaps preventing
...
1000 MCMC iterations, burning in 500 and thinning by 10. We also used the
built-in Vecchia approximation with a conditioning set size of $m=10$. Even with
these limitations, {\sf deepgp} still took a considerable amount of time, and
...
In addition to comparing accuracy and uncertainty quantification through RMSE
and CRPS, we aimed to evaluate the speed of execution for all competitors
involved. Two factors exist that contribute to a computational bottleneck: the
dimension of the simulator response, which has been the main focus of our work,
and the number of runs of the computer model available for training. To test
scaling dimension, we varied the dimensionality of the response from 200 to
20,000, while keeping the number of runs constant. For the two methods that are
most equipped for large dimensions (Scaled Vecchia and SEPIA), we attempt to
stretch their capacity by cranking up the length of the response vector from
20-75K, with steps of 5K. At each dimension size, we ran five Monte Carlo
iterations for each method and recorded the elapsed time.
...
response. {\sf deepgp}'s need to decompose an $n \times n$ matrix at each MCMC
iteration, even aided by the Vecchia approximation, causes it's cost to quickly
grow as dimensionality expands. {\sf laGP}, while built for prediction with
large-scale datasets, does better but is unable to keep pace at a certain point,
likely due to the need to fit a mounting number of local GPs. Reducing the
neighborhood size (set to the default of 50 in our experiment) for {\sf laGP}
could certainly aid in the computation expense, but that would further degrade
it's performance, which already lags being Scaled Vecchia and SEPIA (Figure
\ref{f:ibex_surr_metrics}). The comparison between Scaled Vecchia and SEPIA is
more nuanced. In the right panel of Figure \ref{f:ibex_surr_timing_dim}, we see
that for a fixed value of $m$, Scaled Vecchia's runtime increases linearly with
expanded dimensionality, as advertised by \citet{katzfuss2021general}. For a
fixed number of principal components, SEPIA's execution time increases only
slightly as dimension expands, showcasing it's utility. However, for response
dimension less than 75K, Scaled Vecchia with $m=25$ still executes faster. And
that may be the largest value of $m$ we need. Referring back to Figure
\ref{f:ibex_surr_metrics}, it's clear that increasing conditioning set size or
number of principal components doesn't significant improve performance. It must
also be pointed out that the IBEX simulator surface is not particularly complex
over the model parameters, allowing SEPIA to perform well with a low number of
principal components. That may not be the case in other situations, requiring a
more principal components and by extension a heavier computational load. We'll
see one more comparison that illustrates Scaled Vecchia's superior performance
in our next experiment.

%% NEXT EXPERIMENT
Here we consider modifying the size of the training dataset (i.e, the number of
simulator runs $n$). SEPIA was specifically built for computer experiments with
...
univariate response. We show this robustness in our next bakeoff. Our experiment
is set up similar to the previous one, but we keep the dimensionality of the
response at 10K, while sliding the number of simulator runs from 10 to 100.
Again, each method is run over five MC iterations for each value of $n$. For the
higher performing methods, we push the campaign size up further up $n=2500$.
Results are shown in Figure \ref{f:ibex_surr_timing_size}.

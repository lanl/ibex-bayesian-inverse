...
observation) and a computer implementation of a physical model $m(\cdot)$ with
unknown parameters $\boldsymbol{u}$. Given prior information about
$\boldsymbol{u}$, we want to know which $\boldsymbol{u}$'s lead to
realizations of $m(\cdot)$ that could explain $Y^F$, i.e., the posterior for
$\boldsymbol{u} \mid Y^F$. This is a classic Bayesian inverse problem
\citep{kaipio2011bayesian,stuart2010inverse,knapik2011bayesian}.  Figure
\ref{f:inv_v_classic} provides a diagram, contrasting classical Bayesian
inference with our computer model inverse problem setting.  

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.16,trim=0 0 0 0,clip=TRUE]{inv_v_classic.pdf}
\caption{An illustration of classic Bayesian statistics (top)
and the approach to Bayesian Inverse Problems (bottom). Output $Y^F$ is
Gaussian in either case, but parameters $\boldsymbol{\theta}$ differ in where
they originate. Note that the ``example'' for Bayesian Inverse Problems is
also comprehensive, as it is the only context addressed in the current
literature.
\label{f:inv_v_classic}}
\end{figure}

One big difference with Bayesian inverse problems, as opposed to classical
statistical modeling, is that the parameters $\boldsymbol{\theta}$ defining
the distribution of $Y^F$ are not of direct interest. Rather, we are
interested in the settings $\mathbf{u}$ that define $\boldsymbol{\theta}$
through $m(\cdot)$, which depends on $\mathbf{u}$ and whatever spatial field,
or grid, $X^F$ determines it's values.  For example, two different settings of
$\mathbf{u}$ were used to the center and right panels of Figure \ref{f:fig1}.
Those colors indicate output $\boldsymbol{\theta}$ values on a grid $X^F$ of
longitude and latitude. We are interested in learning about which $\mathbf{u}$
are likely to have generated the observed $Y^F$ values in the left panel.
\textcolor{red}{I'm not loving the disconnect between u, $\theta$, and $X^F$,
$Y^F$ and a dense predictive grid $X$, which is not mentioned but is clearly
being used in these visuals.}
...
$Y^F$, observed in replicate under some true, unknown process at
different field locations $X^F$.  A computer simulation $m(\cdot)$ was built
to virtualize that process under a physical model with two unknown parameters
$\boldsymbol{u} = (u_1,  u_2)^\top$.  Each light gray line is a realization
of $m(\cdot)$ for a different $\mathbf{u}$, whereas the bold-black-dashed
line is $m(\cdot)$ at the (presumed unknown) truth.  

% Needs to go earlier/or change
% The black, bold, dashed line shows the underlying process
% $m(\cdot)$ when $\boldsymbol{u}$ is set to the truth. It is important to point out
% that both $X^F$ and $\boldsymbol{u}$ are arguments to the simulator. But they differ
% in that $X^F$ includes inputs that are modifiable in both field and simulated
% environments, whereas $\boldsymbol{u}$ can only be controlled and ``measured'' in a
% computer model. 

% Furthermore, in our realm of high-dimensional responses, $X^F$ simply
% act as indices, or grid points, for the output. For IBEX, they are specifically
% latitude/longitude pairs.
\begin{figure}[ht!]
\centering
\includegraphics[scale=0.51,trim=0 0 26 0,clip=TRUE]{logit1_obs.pdf}
\includegraphics[scale=0.51,trim=30 0 26 0,clip=TRUE]{logit1_est.pdf}
\includegraphics[scale=0.51,trim=0 0 26 0,clip=TRUE]{logit1_post_draws.pdf}
...
The middle panel of the figure shows posterior draws of $\mathbf{u}$ obtained
under Metropolis sampling using a multivariate normal likelihood whose mean is
$m(\cdot)$ and whose covariance is $\sigma^2 \mathbb{I}$, where $\sigma^2$ is
marginalized out under a reference prior. \textcolor{red}{Steven, some of
these lines look really improbable to me.}  These are not of direct interest
in and of themselves.  Rather, the main object of interest is summarized in
the right panel: the posterior samples of $\mathbf{u}$.  \textcolor{red}{It would be great if this could mode directly connect back to Figure \ref{f:fig1}.}

% To perform inference for $\boldsymbol{u} \mid Y^F$, we follow Bayes' rule,
% $\pi(\boldsymbol{u}|Y^F) \propto
% \mathcal{L}(Y^F|\boldsymbol{u})\pi(\boldsymbol{u})$, and utilize
% Metropolis-Hastings (MH) to sample from the posterior. At each iteration
% $t$, we propose a new set of simulation parameters $\boldsymbol{u}^*$ and
% execute the computer model $\left(m(\boldsymbol{u}^*)\right)$ to get
% $\boldsymbol{\theta^*}$. Then, we evaluate the MH-ratio and either accept or
% reject the proposed values. Predictions and posterior estimates for our
% example are shown in the middle and right panels of Figure
% \ref{f:toy_calib}. Note that each of the light gray lines in the middle
% panel represents a single run of the simulation at one of the accepted set
% of model parameters, $\boldsymbol{u}^{(t)}$. That is, each gray line
% corresponds to a circle in the right panel, where posterior draws of
% $\boldsymbol{u}$ are displayed. It is clearly evident that the MH sampler
% thoroughly explores the input space and that it successfully captures the
% true value of $\boldsymbol{u}$.

The literature on Bayesian inverse problems involves methods that work
predominantly in this way, particularly as regards the Gaussian likelihood. 
%
% Now, one drawback to the current literature is that modeling is predominately
% Gaussian (as illustrated in Figure \ref{f:inv_v_classic} and our toy example),
% assuming $Y^F \sim \mathcal{N}\!\left(\mu\left(X^F\right),
% \Sigma\left(X^F\right)\right)$ as a likelihood, where
% $\boldsymbol{\theta}(\boldsymbol{u})=(\mu\left(X^F\right),
% \Sigma\left(X^F\right))$ depends on $m(\boldsymbol{u})$. 
%
Yet our IBEX observations are counts. Our first contribution, which is albeit
rather straightforward, is to depart from that literature by perusing a
Poisson likelihood: $Y^F \sim Pois(\boldsymbol{\lambda}(\boldsymbol{u}))$,
where $\boldsymbol{\lambda}(\boldsymbol{u})$ is determined by
$m(\boldsymbol{u})$. We leave the details of this to Section
\ref{sec:gen_bayes_inv}.

Another challenge to the conventional apparatus involves computational
considerations.  Each run of the IBEX model $m(\cdot)$ is a Herculean effort,
requiring thousands of core hours on a supercomputer.  That simply can't be
embedded in a Metropolis sampler requiring thousands of iteration to
convergence.  While it is rather conventional to deploy a surrogate model
\citep[e.g.][]{gramacy2020surrogates} in this setting, trained on a design of
space-filling $\mathbf{u}$-values, doing so effectively is challenged by the
dense grid of output, over longitude and latitude via $X^F$ for each
$\mathbf{u}$.  Herein lies our main methodological contribution. By way of
explaining, the remainder of this section outlines the conventional surrogate
modeling toolkit, as a jumping-off point toward explaining both our main
comparative (Section \ref{sec:pca}), and our main methodological contribution.

% Another mismatch involves the size of the spatial indexing set $X^F$ involved
% in IBEX.  Each simulation $m(\cdot)$ outputs $\boldsymbol{\lambda}$ values 
% on a dense longitude and latitude grid.  
% The most significant hurdle that we have not addressed in our toy example is the
% expense of the computer model. Even with modern-day computing power, some simulations
% can take hours, days, or weeks to complete. So, it's not practical to run $m(\cdot)$
% inside the MH sampler. This constrains the amount of runs that can be executed and
% therefore restricts the exploration of the model parameter space. Therefore, we need
% a model (what we call a surrogate, or emulator) fit to the limited simulator output
% to make accurate and reliable predictions for out-of-sample values of our computer
% model parameters. Any statistical model can serve as a surrogate, ranging from linear
% regression to complex neural networks. However, the canonical surrogate for computer
% experiments has come to be the Gaussian process \citep{gramacy2020surrogates}.
...
A Gaussian process is a realization of a multivariate normal distribution, governed
by some mean and covariance function. GPs are fit to observed data and used to make
predictions at new input locations, relying on the conditional multivariate normal or
\textit{kriging} equations \citep{matheron1963principles}. For example, suppose we
have a vector of observations $Y^M$ at input locations $X^M$. Here we are using the
superscript $M$ to make it explicit that our GP surrogate is modeling the simulator
input and output, not field data. $Y^M$ is an $n \times 1$ vector and $X^M$ is an $n
\times p$ matrix. To connect back our model parameters of interest, note that $X^M =
\begin{bmatrix} X^F & \boldsymbol{1}_n \boldsymbol{u}^T \end{bmatrix}$. We make the
assumption that $Y^F$ is normally distributed with some mean $\mu(X^F)$ and
covariance $\Sigma(X^F)$, clearly both functions of $X^F$. Then, predicting at a new
location $x$ is relatively straightforward. Equation~\ref{eq:mvn_eq} articulates how to
obtain mean predictions $\mu^*(x)$ and corresponding uncertainty quantification
estimates $\Sigma^*(x)$.
\vspace{-0.7cm}

\begin{minipage}[t]{0.25\linewidth}
\begin{align}
Y^M(X^M) &\sim \mathcal{N}\!\left(\mu(X^M), \Sigma(X^M)\right) \nonumber \\
y^*(x) \mid Y(X^M) &\sim \mathcal{N}\!\left(\mu^*(x), \Sigma^*(x)\right) \nonumber
\end{align}
\end{minipage}
\quad
\begin{minipage}[t]{0.58\linewidth}
\begin{align}
\mu^*(x) &= \mu(x) + \Sigma(x, X^M) \Sigma(X^M)^{-1} (Y^M - \mu(X^M)) \nonumber \\
\Sigma^*(x) &= \Sigma(x, x)
  - \Sigma(x, X^M) \Sigma(X^M)^{-1} \Sigma(X^M, x) \hspace{0.05em}
\label{eq:mvn_eq}
\end{align}
\end{minipage}
\vspace{0.7cm}
...
common to set $\mu(X^M)=0$. In practice, this simply implies centering $Y^M$ by
subtracting off the sample mean, $\overline{Y}^M$. In this zero-mean context,
...
Many covariance kernels exists, such as the Gaussian, Exponential, or Matern.
We use the Matern kernel in our work, of which both the Gaussian and
Exponential kernels are special cases. The behavior of these kernels is
determined by three hyperparameters: a nugget $g$, lengthscale $\theta$, and
scale $\tau^2$. All hyperparameters can be estimated using your preferred
method (i.e, maximum likelihood, method of moments). We will use Bayesian
methods, specifically MCMC.

GPs provide a flexible, nonparametric regression tool. In addition to computer
experiments, they are utilized in geostatistics \citep{matheron1963principles},
spatial statistics \citep{cressie2011statistics}, time series
\citep{roberts2013gaussian}, optimization \citep{jones1998efficient}, and machine
learning \citep{rasmussen2003gaussian}. The reason for their popularity as surrogate
models in the world of computer experiments is their ability to interpolate between
observed data points (i.e, setting nugget $g = 0$). Although stochastic computer
models are becoming more common, computer experiments are often deterministic,
motivating the desire for a model that interpolates. GPs are useful by providing
predictive surfaces that give higher estimates of variance away from observed data,
and virtually no variance at observed locations. Perhaps the hallmark of GPs is that
they naturally provide excellent uncertainty quantification for out-of-sample
predictions, a matter of importance to scientists exploring these computer models.
Therefore, GPs are widely used in Bayesian Inverse Problems.

One downside to this setup, especially for IBEX, is that the output ``images'' are
massive ($> 15,000$ pixels). Historically, it has not been possible to fit a GP to
such large simulation output, producing high-resolution 2d (or 3d) images for each
$m(\boldsymbol{u})$ that is run. Next, we review what has been done for applications
such as IBEX in the past and lay the groundwork for our proposed advancement
introduced in Section \ref{sec:vecchia}.

%------------------------------------------------------------------------------
%% SECTION 2.2 - Principal Components for High-Dimensional Response
\subsection{Principal Components for High-Dimensional Response}
\label{sec:pca}
%------------------------------------------------------------------------------
...
In the early 2000s, \citet{higdoncalib2008} faced the challenge at LANL of estimating
the parameters of a physical process from which they could only gather a small set of
observations. For the purpose of further investigating the process, scientists
represent the process with a simulation. However, not only are these computer models
expensive, producing a limited number of runs, their output is extremely
high-dimensional, in the form of images, shape description, time series, etc. So, in
order to efficiently fit a surrogate model to the simulator, they perform an
approximation to decrease the size of the data. As a solution, they employ principal
components analysis (PCA) to reduce the dimensionality of the computer model
response. Specifically, they represent the vector output of a simulator model as the
sum of $n_k$ basis functions $\boldsymbol{k}_1,
\boldsymbol{k}_2, \ldots ,
\boldsymbol{k}_{n_k}$, each scaled by a corresponding weight $w_i$. They specify
\begin{align}
...
\label{eq:higdon_pca}
\end{align}
where $\boldsymbol x$ is a vector of field locations (a row of $X^F$) and
$\boldsymbol u$ is the same vector of model parameters we've referred to throughout
this review. $\Sigma_{w_i}$, dependent only on a limited number of combinations of
$\boldsymbol x$ and $\boldsymbol u$, is a $n_k \times n_k$ matrix. Hence, the
decomposition of $\Sigma_{w_i}$ avoids the $O(n^3)$ bottleneck in computation when
scaling to large $n$ and can be completed in a reasonable amount of time. Instead of
modeling the high-dimensional response $\boldsymbol y$, the GP only models the small
set of component weights. Bayesian inference via MCMC is used to obtain the
posterior distribution of these component weights. Then, combining estimates of the
weights with the calculated basis functions allow for out-of-sample predictions. As
a result of this approximation, the dimension of the covariance matrices $\Sigma(X^F)$
and $\Sigma(x^*)$ in Eq.~\ref{eq:mvn_eq} remains small and is able to be decomposed
quickly.
...
that our more straightforward approach results in equal or improved
performance in both predictive accuracy and uncertainty quantification, along
with a significant reduction in computational expense. Additionally, their
method and implementation, originally written in {\tt MATLAB} and now deployed
in {\tt Python} \citep{gattiker2020lanl}, is unable to model a non-Gaussian
field response, leading to our first contribution.
...
framework can easily be generalized to any response. In Bayes' rule, we use a
Poisson likelihood, and substitute $\boldsymbol \lambda$ for $\boldsymbol
\theta$, as shown below.
...
introduced in \ref{sec:review}. Instead of observing the function $f(X)$
...
Figure 3. As described in \ref{sec:review}, we know that $f(X)$ is a
...
\ref{sec:pca}. Next, we will propose a modern, intuitive, more

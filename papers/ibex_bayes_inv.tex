...
%------------------------------------------------------------------------------
%% SECTION 3 - Vecchia Approximation for IBEX Simulations
...
simulator (we will circle back to the data collected by the satellite in
Section \ref{sec:sims}). Neighborhoods of size $m \in \{25,50,75,100\}$ are
generated for a particular simulator observation $Y_i^M$ (denoted by a red
star) from the IBEX computer model. It's difficult to visualize in higher
dimensions, so we break the input space down into separate 2d plots, one for
grid space (latitude/longitude) and one for model parameter space ($u$). But to
be clear, points of the same shape and color in both plots are elements of the
same overall conditioning set. For each conditioning set $h_j(i)$, all points
from sets where $m < m_j$ are also included in $h_j(i)$. That is, green circles
from the neighborhood with $m=25$ would also be part of the conditioning set
with blue squares ($m=50$). For reference, we've overlaid the points in grid
space on a grayed-out simulated sky map corresponding with the observation of
interest. Note that more points are concentrated near the reference point in
latitude and longitude, whereas the neighborhood is more spread out over the
unknown model parameters. This behavior is present in all conditioning sets in
IBEX. Therefore, observations are depending on information in close proximity
in grid space, but potentially distant in $u$-space. Furthermore, as $m$ grows,
additional points in grid space are mostly farther away than those already
included, but we don't observe the same trend in $u$-space.
...
or $X^F$). Model parameter ($u$) space is displayed in the right panel, with jitter added, allowing all points to be visible.
...
panels displayed with thin, solid borders). A Scaled Vecchia GP surrogate is
fit to all 66 simulated runs. Then, we predict the output for five unobserved
...
expect. But we are not satistfied with simply passing the eye test, so we turn
to empirical measures and show that our surrogate fit is extremely accurate and
...
In order to validate performance of our surrogate, we conduct experiments to
gauge predictive accuracy, uncertainty quantification, and computational
efficiency. We evaluate how well the model does using root mean square error
(RMSE), continuous ranked probability score (CRPS)
\citep{gneiting2007strictly}, and fitting/prediction time. For the purpose of
comparison, we carry out the same experiment for three competitors: {\sf R}
packages {\sf laGP} \citep{gramacy2014lagp} and {\sf deepgp} \citep{deepGP},
and the current ``status-quo,'' SEPIA \citep{gattiker2020lanl}, implemented in
{\sf Python}.
...
experiment, we fit a Scaled Vecchia approximated GP surrogate on 65 of the
$n=66$ model runs available. Then, we predict at the held out model parameter.
For now, just pay attention to the boxplots left-hand side of each panel, where
$n_k=3$ for SEPIA and $m=25$ for Scaled Vecchia. We'll come back to the others
in a moment. It is obvious that the Scaled Vecchia approximation outperforms
{\sf laGP} and {\sf deepgp} in both RMSE and CRPS. We gave {\sf laGP} the
default neighborhood size of 50, but each prediction is produced via an
independently fit GP, perhaps preventing understanding of global trends in the
surface. {\sf deepgp} is fully Bayesian, sampling from the posterior
distribution via MCMC. But that comes with a cost. Because of the significant
...
conditioning set size of $m=10$. Even with these limitations, {\sf deepgp}
still took a considerable amount of time, and likely contributed to it's poor
...
runs constant. For the two methods that are most equipped for large dimensions
(Scaled Vecchia and SEPIA), we attempt to stretch their capacity by cranking up
the length of the response vector from 20-75K by steps of 5K. At each
dimension size, we ran five Monte Carlo iterations for each method and recorded
...
the neighborhood size for {\sf laGP} could certainly aid in the computation
expense, but that would further degrade it's performance, which already lags
being Scaled Vecchia and SEPIA (Figure \ref{f:ibex_surr_metrics}). The
comparison between Scaled Vecchia and SEPIA is more nuanced. In the right panel
of Figure \ref{f:ibex_surr_timing_dim}, we see that for a fixed value of $m$,
Scaled Vecchia's runtime increases linearly with expanded dimensionality, as
...
each panel in Figure \ref{f:ibex_surr_metrics} show that increasing
conditioning set size or number of principal components doesn't significantly
improve performance. It must also be pointed out that the IBEX simulator
surface is not particularly complex over the model parameters, allowing SEPIA
to perform well with a low value of $n_k$. That may not be the case in other
situations, requiring more principal components and by extension a heavier
computational load. We'll see one more comparison that illustrates Scaled
Vecchia's superior performance in our next experiment.
...
for use in Bayesian inverse problems. In fact,
\citet{scaledvecchiakatzfuss2022} foreshadowed this, suggesting their method
could be used successfully in this context. We take their advice and apply it
to the IBEX problem next.

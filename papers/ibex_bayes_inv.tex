...
observed. Before before diving in, we dispence with a few important details
about IBEX observations and computer simulations.  We then test our modeling
...
perpendicular mean free path. These are are our $u$ values. The response from
...
% \textit{ratio}. But first we wish to validate our proposed methodology.
% Therefore, we run our Poisson Bayesian inverse problem framework on synthetic
% satellite data where we know the ``true'' parameter $u^\star$ and can evaluate
% whether our method is able to recover it before shifting our focus to the
% actual ENA counts collected by the IBEX satellite.
...
Space scientists at LANL have access to over 15 years of data collected by the
IBEX satellite and the resulting sky maps. However, as mentioned above, we only
use satellite data from the years 2009-2011 due to alignment with the
implementation of the computer model, leaving the remaining data for future
work (\ref{sec:discuss}). The number of records in each sky map varies across
the three years from $\sim$7-8K observations, therefore we have over 23K rows
of field data ($X^F, Y^F$) available to use for analysis.
...
difference from Section \ref{sec:synth_data} is that field data ($Y^F, X^F$)
are not exclusively from one map (Figure \ref{f:ibex_real_ex}). Instead, ($Y^F, X^F$) is formed by stacking
the counts, exposure times, background rates, and Cartesian coordinates $x$,
$y$, $z$ from years 2009-2011.
...
particular, the intensity of the ribbon in the surrogate prediction near
latitude $-30^\circ$ and longitude $270^\circ$ appears to be much lower than
what is observed in the satellite data for each year. There's also a gap (in
this instance overestimating the ENA rate) between surrogate and reality
between longitudes $60^\circ$ and $120^\circ$ and latitudes $\pm50^\circ$.
Another another indication of this clash is visible in the right panels of
Figure \ref{f:ibex_real_ex}. Note that these both display the same posterior
distribution, but differ in their axis limits. Posterior mass is concentrated
in the extreme corner of the model parameter space. Our priors attempt to push
sampling away from the boundaries, but the likelihood dominates the posterior.
We take this as an indication that the computer model is lacking in it's
attempt to represent reality, and the Metropolis sampler seeks to go beyond the
range of model parameters present in the training data.

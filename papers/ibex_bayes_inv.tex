...
posterior draws {\color{red}$u^{(t)}$ ($t$ indicating the $t^\mathrm{th}$ step of the
...
$X^M$ remains small, including only the unique combinations of unknown
...
to all {\color{red}$n_U=66$} simulated runs. Then, we predict the output for five unobserved
...
experiment, we fit {\color{red}an SVEC} GP surrogate on 65 of the {\color{red}$n_U=66$} model
runs available. Then, we predict at the held out model parameter. For now,
just pay attention to the boxplots on the left-hand side of each panel. We'll
come back to the others in a moment. It is obvious that {\color{red}SVEC}
outperforms {\sf laGP} and {\sf deepgp} in both RMSE and CRPS.  
{\color{red}Additional visual aids provided in Appendices
...
size of the training dataset (i.e., the number of simulator runs {\color{red}$n_U$}). SEPIA was
...
methods are run over five MC iterations for each value of {\color{red}$n_U$}. For the higher
performing methods, we push the campaign size up further {\color{red}($n_U=2500$)}.
...
Using the corpus of {\color{red}$n_U=66$} computer model runs, varying a design of
\textit{parallel mean free path} and \textit{ratio} $u$-values, described in
Section \ref{sec:vecchia}, we select a value $u^\star$ and set the
corresponding output ($\lambda^\star = m(u^\star, X^M)$) as the
{\color{red}``unknown''} underlying mean ENA rate (upper right panel of Figure
...
values of ratio {\color{red}($n_U=66$)}. The spread of parameter values is
determined by space scientists to include all values considered reasonable:
parallel mean free path ranges from 500-3000 astronomical units (AU); ratio
values are unitless, and span from 0.001 to 0.1. Due to the inability for the
model to furnish posterior samples beyond the boundary parameters, we do not
test those combinations for which at least one parameter is on its edge (i.e.,
when parallel mean free path $\in \{500, 3000\}$ and/or ratio $\in \{0.001,
0.1\}$). Removing these edge cases results in 36 remaining test runs. For each
combination, we follow the same procedure as above: select one model parameter
and set as $u^\star$, create simulated observed satellite data, fit
{\color{red}an SVEC} surrogate on training data excluding $u^\star$, and run
Algorithm \ref{alg:gibbs} to sample from the posterior $u \mid Y^F$. Results
are shown in Figure \ref{f:synth_estimates_all}.
...
developed enhance uncertainty quantification, perhaps with the use of
a deep Gaussian process to handle the inherent nonstationarity in the
discrepancy.}

{\color{red}Section \ref{sec:gp_approx} details several GP approximations for modeling
large-scale datasets. In particular, we review SEPIA \citep{gattiker2020lanl},
a {\sf Python} library based on work by \citet{higdoncalib2008}, and
subsequently use it as a competitor throughout the manuscript. SEPIA performs
comparably to SVEC in terms of predictive accuracy and uncertainty
quantification. However, it suffers computationally as the number of computer
model runs increases. One intriguing extension could involve combining
\citeauthor{higdoncalib2008}'s PCA approach with Vecchia. 
% PCA weights could be modeled using the Vecchia approximation and alleviate a
% growing number of computer model runs. 
To our knowledge this has not been
attempted, but represents a promising area of research.}
...
%. It has been used in
%multiple academic works 
\citep{chan2024constructing,surer2022uncertainty}.
Variational approaches \citep{opper2009variational,hensman2015scalable} could
also be explored. These methods offer additional modeling and computational
options that may be appropriate depending on the objectives and requirements
of a particular application.} % We refer interested readers to the corresponding
% articles and software documentation for detailed descriptions.}

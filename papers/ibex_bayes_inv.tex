...
In 1988, \citeauthor{vecchia1988estimation} proposed an approximation for the
multivariate normal likelihood function. The Vecchia approximation relies on the
ability to write the likelihood as the product of univariate normal conditional
likelihoods, as displayed in Equation \ref{eq:vecchia}.
...
a size $m$, as specified by the user. This induces sparsity in the precision
matrix, potentially dramatically speeding up the evaluation of the likelihood
for large values of $n$. The Vecchia approximation takes an operation of order
$O(n^3)$ down to $O(nm^3)$, motivating small values of $m$.
\begin{align}
...
\label{eq:vecchia}
\end{align}
This approximation has been recently popular due to work from
\citep{katzfuss2020vecchia, katzfuss2021general},
% zhang2022multi, sauer2023vecchia}.
Logically, as the size of $h(i)$ decreases,

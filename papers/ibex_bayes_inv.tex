...
Tech} \and Lauren J. Beesley\thanks{Statistics Group, Los Alamos
...
take inputs that can be varied to modify the shape and intensity of the ribbon.
The ribbon-only model relies on two parameters, \textit{parallel mean free
path} and \textit{ratio}, while the GDF model we use remains constant and is
simply added on to the output of the ribbon model. Dialing in good settings for
these input parameters, in light of observed ENA counts from IBEX, is
...
hydrogen ion travels beyond the heliopause before interacting with another
particle, receiving an electron, and becoming an ENA. The \textit{ratio}
parameter is the fraction of \textit{parallel mean free path} and another
measure of distance, perpendicular mean free path. These are our $u$ values.
The response from the GDF model does not vary with any parameters of interest
and is merely added to the output of the ribbon simulator.

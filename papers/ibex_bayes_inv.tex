...
$u$ of a computer model that produce a (simulated) sky map most likely to have
generated some observed counts. But before diving in, we need to describe a few
nuances we have not yet revealed about the data collected by the IBEX satellite
and some particular implementation details of the computer model.
...
\citep{zirnstein2021dependence,zirnsteinGDFsims2015}, we give some basic
context here. We focus on two particular models available (among many), one for
the GDF and one for the ribbon. In the ribbon model, the parameter
...
\textit{parallel mean free path} and another measure of distance, perpendicular
mean free path. The response from the GDF model does not vary with any
parameters of interest and is simply added to the output of the ribbon
simulator.
...
satellite data where we know the ``true'' parameter $u^*$ and can evaluate
...
model parameter $u^*$ at the nominal 95\% rate when provided with synthetic

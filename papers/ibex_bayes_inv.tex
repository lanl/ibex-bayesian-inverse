...
where parameters $u$ are unknown and $X$ represents a dense grid at which to
evaluate the simulator. Given prior information about $u$, we want to know
which $u$'s lead to realizations of $m(\cdot)$ that could explain $Y^F$, i.e.,
the posterior for $u \mid Y^F$. This is a classic Bayesian inverse problem
\citep{kaipio2011bayesian,stuart2010inverse,knapik2011bayesian}. Equation
\ref{eq:inv_bayes_mod} provides a hierarchical model, contrasting classical
Bayesian inference with our computer model setting.
\begin{align}
Y^F &\sim \mathcal{N}_{n_F}\!\left(\mu\!\left(X^F\right),
\Sigma\!\left(X^F\right)\right) \nonumber \\
\mu(X) &= m(u, X) \nonumber \\
\Sigma(X) &\sim \pi\left(\Sigma\right) \nonumber \\
u &\sim \pi(u)
\label{eq:inv_bayes_mod}
\end{align}
One big difference with Bayesian inverse problems, as opposed to classical
statistical modeling, is that the parameters $\mu(X^F)$ and $\Sigma(X^F)$
defining the distribution of $Y^F$ are not of direct interest. Rather, we are
interested in the settings $u$ that define $\mu(X)$ and $\Sigma(X)$ through
$m(u, X)$, which depends on $u$ and whatever spatial field, or grid, $X$
determines it's values.  For example, two different settings of $u$ were used
in the IBEX simulator to generate the center and right panels of Figure
\ref{f:fig1}. Colors indicate output $\mu(X)$ values on a dense grid $X$ of
longitude and latitude. We are interested in learning which $u$ are
likely to have generated the observed $Y^F$ values in the left panel at
locations where the IBEX satellite points ($X=X^F$).
...
$Y^F$, observed in replicate under some true, unknown process at different
locations $X^F$. A computer simulation $m(u, X)$ was built to virtualize that
process under a physical model with two unknown parameters $u = (u_1,
u_2)^\top$. Each light gray line is a realization of $m(\cdot)$ for a
different $u$ on an identical fine grid $X$. The bold-black-dashed line is
$m(\cdot)$ at the (presumed unknown) truth.
...
normal likelihood whose mean $\mu(X)=m(\cdot)$ and whose covariance $\Sigma(X) \sim \sigma^2
\mathbb{I}$, where $\sigma^2$ is marginalized out under a reference prior.
Although we display these runs on a high-resolution grid $X$, the sampler only uses
evaluations of $m(\cdot)$ at locations $X^F$ to calculate the likelihood. Even
so, these are not of direct interest in and of themselves.  Rather, the main
object of interest is summarized in the right panel: the posterior samples of
$u$. These samples will inform our inference about the nature of the process that generates
observations $Y^F$. In this toy example, it is evident that the Metropolis
sampler successfully captures the true value of $u$ (right panel) after thoroughly exploring
the computer model space (center panel).
...
predominantly in this way, particularly as regards the Gaussian likelihood. We
aim to provide a method that achieves the same goal, but for unknown
parameters $u$ that underlie observed IBEX satellite data (left panel of
Figure \ref{f:fig1}).
%
...
% assuming $Y^F \sim \mathcal{N}\!\left(\mu\left(\boldsymbol{X}^F\right),
% \Sigma\left(\boldsymbol{X}^F\right)\right)$ as a likelihood, where
% $\boldsymbol{\theta}(\boldsymbol{u})=(\mu\left(\boldsymbol{X}^F\right),
% \Sigma\left(\boldsymbol{X}^F\right))$ depends on $m(\boldsymbol{u})$.
%
Yet our IBEX observations are counts. Our first contribution, which is albeit
rather straightforward, is to depart from that literature and
Eq.~\ref{eq:inv_bayes_mod} by perusing a Poisson likelihood: $Y^F \sim
Poisson(\lambda(X^F))$, where $\lambda(X^F)$ is determined by $m(u, X)$. We
leave the details of this to Section \ref{sec:gen_bayes_inv}.

Another challenge to the typical apparatus involves computational
...
embedded in a Metropolis sampler requiring thousands of iterations to
...
space-filling $u$-values, doing so effectively is challenged by the dense grid
of output, over longitude and latitude via $X$ for each $u$.  Herein lies our
main methodological contribution. The remainder of this section outlines the
standard surrogate modeling toolkit, as a jumping-off point toward explaining
both our main comparative (Section \ref{sec:pca}) and our main methodological
contribution.

% Another mismatch involves the size of the spatial indexing set $\boldsymbol{X}^F$ involved
...
%------------------------------------------------------------------------------
%% SECTION 2.1 - Gaussian Process Surrogates for Computer Experiments:
...
have a vector of observations $Y^M$ at input locations $X^M$. Here we use the
superscript $M$ to make explicit that our GP surrogate is modeling the simulator
input and output, not field data. We make the assumption that $Y^M$ is normally
distributed with some mean $\mu\!\left(X^M\right)$ and covariance
$\Sigma\!\left(X^M\right)$, clearly both functions of $X^M$. Then, predicting at a
new location $x$ is relatively straightforward. Equation~\ref{eq:mvn_eq} articulates
how to obtain mean predictions $\mu^*(x)$ and corresponding uncertainty
quantification estimates $\Sigma^*(x)$.
\vspace{-0.7cm}

\begin{minipage}[t]{0.25\linewidth}
\begin{align}
Y^M(X^M) &\sim \mathcal{N}_{n_M}\!\left(\mu(X^M), \Sigma(X^M)\right) \nonumber \\
...
common to set $\mu(x)=\mu(X^M)=0$. In practice, this simply implies centering $Y^M$ by
...
$m(u, X)$ that is run. Next, we review what has been done for applications
...
In the early 2000s, \citet{higdoncalib2008} faced the challenge at LANL of
estimating the parameters of a physical process from which they could only
gather a small set of observations. For the purpose of further investigation,
scientists commonly represent the process with a simulation. However, these computer models are not only expensive, producing a limited number of runs, but their
output is extremely high-dimensional, in the form of images, shape
description, time series, etc. So, in order to efficiently fit a surrogate
model to the simulator, \citet{higdoncalib2008} perform an approximation to decrease the size of
the data. As a solution, they employ principal components analysis (PCA) to
reduce the dimensionality of the computer model response. Specifically, they
represent the vector output of a simulator model as the sum of $n_k$ basis
functions $k_1, k_2, \ldots , k_{n_k}$, each scaled by a corresponding weight
$w_i$. They specify each model response
\begin{align}
y(x) &\approx \sum_{i=1}^{n_k}  k_i \cdot w_i (x), \quad n_k \ll n \ \nonumber
\\ w_i(x) &\sim \mathcal{N}( 0, \Sigma_{w_i}),
\label{eq:higdon_pca}
\end{align}
where $x$ is a single row of the matrix $X^M$. Note that $X^M$ in this
approach is small, only containing entries corresponding to the unique values
of unknown model parameters ($x = u$) executed by the computer simulation, and no information on the grid
where the simulator is evaluated. Weights $w_i$ are realizations of a GP, fit
over principal components from the limited simulator output. Therefore, by not
directly modeling the high-dimensional response $y$ and it's associated dense
grid $X$, the decomposition of $\Sigma_{w_i}$ avoids the cubic bottleneck in
computation when scaling to large $n$ and can be completed in a reasonable
amount of time. With fitted surrogate in hand, Bayesian inference via MCMC is
used to obtain the posterior distribution of these component weights. Then,
combining estimates of the weights with the calculated basis functions allows
for out-of-sample predictions. As a result of this approximation, the
dimension of the covariance matrices $\Sigma(X^M)$ and $\Sigma^*(x)$ in
Eq.~\ref{eq:mvn_eq} remains small and is able to be decomposed quickly.
...
the input matrix $X^M$ for each response the number of times equal to the
length of the output vector. In essence, implicitly encode the grid points
(e.g.~latitude/longitude pair, pixel in image, moment in time series) of the response vector for each
observation into the input matrix. We believe that had \citet{higdoncalib2008}
had access to the capacity of modern-day computing, along with the novel
methodology proposed since the early 2000s that leverages that compute, they
would have steered their implementation in a significantly different way. We
will show that our more straightforward approach results in equal or improved
performance in both predictive accuracy and uncertainty quantification,
greater flexibility in applications, and a significant reduction in
computational expense. Additionally, their method and implementation,
originally written in {\tt MATLAB} and now deployed in {\tt Python}
\citep{gattiker2020lanl}, is unable to model a non-Gaussian field response,
leading to our first contribution.

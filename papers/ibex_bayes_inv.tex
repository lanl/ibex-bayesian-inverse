...
longitude and latitude. We are interested in learning about which $u$ are
likely to have generated the observed $Y^F$ values in the left panel at
locations $X=X^F$ where the IBEX satellite points.
...
Although we display these runs on the grid $X$, the sampler only uses
...
$u$. These samples will inform our inference about the process that generates
observations $Y^F$. In this toy example, it is evident that the Metropolis
sampler successfully captures the true value of $u$ after thoroughly exploring
the computer model space.
...
scientists represent the process with a simulation. However, not only are
these computer models expensive, producing a limited number of runs, their
...
model to the simulator, they perform an approximation to decrease the size of
...
of $x = u$ executed by the computer model, and no information on the grid
...
combining estimates of the weights with the calculated basis functions allow
...
(e.g.~pixel in image, moment in time series) of the response vector for each

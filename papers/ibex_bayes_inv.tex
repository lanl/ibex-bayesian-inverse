...
A Gaussian process \citep[e.g.,][]{rasmussen2003gaussian} is a realization of
a multivariate normal (MVN) distribution, governed by some mean and covariance
function. GPs are fit to observed data and used to make predictions at new
...
suppose we have a vector of observations $Y^M$ at input locations $X^M$. Here
we use the superscript $M$ to make explicit that our GP surrogate is modeling
the simulator input and output, not field data. Suppose $Y^M$ is MVN with mean
$\mu\!\left(X^M\right)$ and covariance $\Sigma\!\left(X^M\right)$. 
Then, predicting at a new location $x$ follows
\begin{align}
...
context, all the action takes place in the covariance function $\Sigma(x,
x')$, which is usually specified as inversely poportional to distances between
inputs $x$ and $x'$. We use a form known as the separable Mat\'ern kernel
\citep{stein1999interpolation} in our work, of which both the Gaussian and
Exponential kernels are special cases perameterized a nugget $g$, lengthscales
$\theta$, and scale $\tau^2$, whose sampling is included along with others in
posterior integration via MCMC. GPs provide a flexible, nonparametric
regression tool. In addition to enjoying a canonical stats as surrgates for
computer simulation experiments experiments, they are utilized in
geostatistics geospatial statistics \citep{cressie2011statistics}, time series
\citep{roberts2013gaussian}, optimization \citep{jones1998efficient}, and
machine learning \citep{rasmussen2003gaussian}.

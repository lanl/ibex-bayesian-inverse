...
example, the computer mode $m(\cdot, \cdot)$ was used to generate the data.
In practice, the model represents a characacure using known or best-guess
...
line is $m(u^\star, X)$ provided as a referece.
...
\includegraphics[scale=0.55,trim=20 0 30 0,clip=TRUE]{logit1_post_draws.pdf}
...
generate a Gaussian or continuous response. For instance, a field experiment
may return a binary response and a corresponding computer outputs the
probability of success or failure at each location. Categorical responses may
expand beyond two classes, representing a multinomial response. In this paper
we focus on Poisson-distributed counts in the field and an associated simulator
...
u &\sim \pi(u), & \mbox{e.g.,} \quad u &\sim \mathrm{Unif}[0,1]^p \nonumber
\end{align}
where $e(X^F)$ represents exposure time, or the duration the satellite pointed
at a specific location in the sky. Therefore, $\lambda(X^F)$ denotes the rate
at which ENA particles are generated for a row (latitude/longitude pair) in
$X^F$. We note that it is common to assume unit exposure ($e_i=1$) when working
with count data. However, in our application we are not given unit exposure,
but each data point is a total count of particles observed over a series of
different time intervals. As mentioned previously, we (or the scientists we
collaborate with) are not necessarily interested in the posterior distribution
of $\lambda(X)$. We are looking to understand the posterior $u \mid Y^F$, where
$\lambda(X^F)$ is determined by a computer simulation $m$. Our MCMC approach to
learning $u \mid Y^F$ is summarized in our first algorithm.
\begin{algorithm}[ht]
\DontPrintSemicolon
Initialize $u^{(0)}$. \\
...
m(u^*, X^F)$, as shown in the left panel of Figure 3. We know that
$m(\cdot,\cdot) = \lambda(X)$ is a function of a high-resolution grid $X$ and
two unknown parameters $u=(u_1, u_2)$. We also have associated exposure times
with each $X^F$ location. Counts for individual exposure times are shown as red
stars. Red circles, indicating the total count at an input location divided by
the total time observed at that location, represent the actual data we receive.
Results from running Algorithm \ref{alg:gibbs} are shown in the middle and
right panels of Figure \ref{f:toy_calib_pois}. Similar to the results in Figure
\ref{f:toy_calib}, it is clear that the sampler searches a broad swath of the
model input space but hones in on the region where the true parameter values lie.
\begin{figure}[ht!]
\centering
...
\includegraphics[scale=0.55,trim=20 0 30 0,clip=TRUE]{logit1_pois_post_draws.pdf}
...
inverse problem apparatus. Each run of the IBEX model $m(\cdot, \cdot)$ is a
Herculean effort, requiring hundreds or thousands of core hours on a
...
effectively is challenged by the dimension of $X^F$.  Herein lies our main
methodological contribution, using a modern sparse approach to surrogate
modeling in a (Poisson) Bayesian inverse problem setting.  The remainder of
this section outlines the standard surrogate modeling toolkit, as a jumping-off
point toward explaining both our main comparative (Section \ref{sec:gp_approx})
and our main methodological contribution.
...
Evaluating the likelihood $Y_F$ in Eq.~(\ref{eq:inv_bayes_mod}), e.g., within
a Metropolis step, may proceed via surrogate when {\em in situ} evaluation
directly on $m(\cdot, \cdot)$ is too slow. Suppose we have a vector of
observations $Y^M$ at input locations $X^M$ from a campaign of runs of
$m(\cdot, \cdot)$ that may or may not coincide with settings in $X^F$. We
prefer a small design of space-filling $u$-values ($U$), paired via Cartesian
product with $X^F$ from the field experiment, or a predictive grid $X$, e.g.,
$X^M = X \otimes U$, depending on the situation.   Two such runs, i.e., for
two rows $u^\top \in U$ and a dense grid $X$, are shown in Figure
\ref{f:fig1}.
...
simulations are based on Gaussian process
...
proposed $u$-value and paired $X^F$) follows $Y(x) \mid Y(X^M) \sim
...
common to set $\mu(\cdot)=0$ assuming pre-centered responses. In this
zero-mean context, all the action takes place in the covariance function
$\Sigma(x, x')$, which is usually specified as inversely poportional to
distances between inputs $x$ and $x'$. We use a form known as the separable
Mat\'ern kernel \citep{stein1999interpolation} in our work, a conventional
choice, parameterized by a nugget $g$, lengthscales $\theta$, and scale
$\tau^2$, whose sampling is included along with others in posterior
integration via MCMC. For details, see \citep[][Chapter
...
GP surrogates can be extremely effective. For example, in Section
\ref{sec:gen_bayes_inv}, Figures \ref{f:toy_calib} and \ref{f:toy_calib_pois}
show a limited sample of computer model evaluations ($n < 50$). For these toy
examples, we ``assumed'' we had unlimited access to the simulator. In reality,
we fit a GP surrogate to the model runs in the left panels. ``Model runs'' in
the center panels are not in fact evaluations of the model, but predictions
from a fitted GP surrogate. Even so, the Metropolis sampler used those
predictions to successfully recover the true model parameters.
...
prohibitive cost. Further complicating this example is that our computer model
output is functional, i.e. each response is a vector. In Section
\ref{sec:gen_bayes_inv}, each simulation was evaluated on a grid ($X$) of 20
unique locations. Therefore, although the number of model runs is small, we are
really modeling 800 ($40 \times 20$) responses. A training set of 800 data
points is already stretching the limitations of a GP (decomposing an $800
\times 800$ matrix). But consider the high-resolution simulated \textit{sky
maps} in the middle and right panels of Figure \ref{f:fig1}. The two degree
grid over latitude and longitude (under the hood the output is in spherical
coordinates $x$, $y$, and $z$) results in ($\gg 10{,}000$ locations, or pixels)
for each $u$-value. Combine that with $n=66$ unique $u$-values and our response
vector will have greater than one million elements. Even if we used a smaller
LHS space-filling design of size $n=15$ over the model parameters, the response
vector would still consist of over 200,000 points. In that case, a GP could not
be fit and provide predictions in a reasonable amount of time.
...
Many methods have been introduced to address this computational burden,
particularly in the past decade as increased access to computing resources has
led to larger and larger training datasets. These methods include, but are not
...
nearest neighbors \citep{datta2016hierarchical, wu2022variational}. Most of our
modeling effort involves circumventing this bottleneck, however there is a
method tailor-made for this setting when the likelihood (for $Y_F$) is
Gaussian.

In the early 2000s, \citet{higdoncalib2008} faced challenges much like the
IBEX simulator. Not only were the computer models they worked with similarly
...
fit a GP surrogate model on such large data. \citeauthor{higdoncalib2008}'s
key insight involved treating $X^F$ (or gridded $X$) -- i.e., the fixed
reference set that a simulator realization is evaluated on for each candidate
parameter $u$ -- as a feature of {\em output}, rather than conventionally an
input. In this way, $X^M$ remains small, including only the unique
...
%  Fitted surrogate in hand, Bayesian
% inference via MCMC is used to obtain the posterior distribution of component
% weights $w_i(u)$. Out-of-sample predictions at each iteration are obtained by
% combining predicted weights $\hat{w_i}(u^*)$ with the calculated basis
% functions $k_i$. Dimension of the covariance matrices $\Sigma(X^M)$ and
% $\Sigma^*(x)$ in Eq.~(\ref{eq:mvn_eq}) remains small and decomposition
% proceeds quickly.
...
Fortunately, GP approximation has come a long way in the last twenty or years.
\citeauthor{higdoncalib2008} specifically lamented not having access to such
technology, necessitating a more complex approach. It is no longer necessary
to play the trick of moving a reference grid to the output space.  A GP
(approximation) may model $Y^M$ via all inputs $X^F$ and $U$ via $X^M$, as we
shall demonstrate next.  The result is at once more modern, faster, and better
compartmentalized for use in novel contexts, such as our own.

% We propose a simpler approach in Section \ref{sec:vecchia}, devoid of the
% definition and selection of how many basis functions to use. Although this basis
% representation was and still is successful in many applications, we argue that
% what \citet{higdoncalib2008} really wanted to do was represent the functional
% output as a univariate response. That is, return to the approach of Section
% \ref{sec:gp_surr} and replicate the entry in the input matrix $X^M$ for each
% unique $u$-value the number of times equal to the length of the output vector. In
% essence, implicitly encode the grid points (e.g.~latitude/longitude pair, pixel
% in image, moment in time series) of the response vector for each observation
% into the input matrix. We believe that had \citet{higdoncalib2008} had access to
% the capacity of modern-day computing, along with the novel methodology proposed
% since the early 2000s that leverages that compute, they would have steered their
% implementation in a significantly different way. We will show that our more
% straightforward approach results in equal or improved performance in both
% predictive accuracy and uncertainty quantification, a significant reduction in
% computational expense., and greater flexibility in applications Additionally,
% their method and implementation, o is unable to model a
% non-Gaussian field response $Y^F$, leading to our first contribution.
...
We believe treating the data as a functional or multivariate response, then
...
data. But as mentioned in Section \ref{sec:gp_approx} there are many GP
approximations available to model the output of a computer simulation in this
manner. We propose using the recently popularized Vecchia approximation
\citep{vecchia1988estimation, katzfuss2020vecchia, katzfuss2021general,
zhang2022multi, sauer2023vecchia}.
\begin{figure}[ht!]
\centering
...
\ref{sec:gp_approx} and see how the Scaled Vecchia approximation
...
%------------------------------------------------------------------------------
%% SECTION 3.1.2 - Assessing predictive accuracy
\subsubsection{Assessing execution time}

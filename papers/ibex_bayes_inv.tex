...
a specific location in the sky. Therefore, $\lambda(X^F)$ denotes a rate at
which ENA particles are generated for rows (latitude/longitude pairs) in $X^F$.
We note that it is common to assume unit exposure ($e(\cdot)=1$) when working
with count data. However, in our application each data point is a total count of
particles observed over a series of different time intervals. As mentioned
previously, we (or the scientists we collaborate with) are not necessarily
interested in the posterior distribution of $\lambda(X)$. We hope to understand
the posterior $u \mid Y^F$, where $\lambda(X^F)$ is determined by a computer
simulation $m(\cdot,\cdot)$. Our MCMC approach to learning $u \mid Y^F$ is
summarized in our first algorithm.
\begin{algorithm}[ht]
\DontPrintSemicolon
Initialize $u^{(0)}$. \\
...
Computer model runs $m(\cdot,\cdot) = \lambda(X)$ produce mean surfaces given
unique values of $u = (u_1, u_2)$. Observed counts are shown as red stars. In
contrast to our previous example, each observation has an associated exposure
time $e(\cdot)$. Red circles indicate the sum of all counts at an input location
(i.e, a single row of $X^F$) divided by the sum of their corresponding exposure
times. These points are the actual data we receive. Results from running
Algorithm \ref{alg:gibbs} are shown in the middle and right panels of Figure
\ref{f:toy_calib_pois}. Similar to the results in Figure \ref{f:toy_calib}, it
is clear that the sampler searches a broad swath of the model input space but
hones in on the region where the true parameter values lie.
\begin{figure}[ht!]
\centering
...
surrogate models such as GPs via inducing points \citep{banerjee2008gaussian}
and local approximate GPs \citep{gramacy2014lagp}, but found them unable to
manage the scale of the IBEX data. Herein lies our main methodological
contribution, using a modern sparse approach to surrogate modeling in a
(Poisson) Bayesian inverse problem setting.  The remainder of this section
outlines the standard surrogate modeling toolkit, as a jumping-off point toward
explaining both our main comparative (Section \ref{sec:gp_approx}) and our main
...
GP surrogates can be extremely effective. To illustrate, recall Figures
...
successfully recover the true model parameters.
...
Further complicating this example is that our computer model output is
functional, i.e. each response is a vector. In Section \ref{sec:gen_bayes_inv},
each run of the simulator was evaluated on a grid ($X$) of 20 unique locations.
Therefore, although the number of model runs is small ($n=40$), we are really
modeling 800 ($40 \times 20$) responses. A training set of 800 data points is
already stretching the limitations of a GP (decomposing an $800
...
coordinates $x$, $y$, and $z$) results in ($> 16{,}000$ locations, or pixels)
...
be fit and provide predictions in a reasonable amount of time, prompting our next section.
...
Many methods have been introduced to address the $O(n^3)$ computational burden
associated with GPs. The prevalence of such methods has grown in the past decade
as increased access to computing resources leads to larger and larger training
...
wu2022variational}. Most of our modeling effort in this paper involves
circumventing this cubic bottleneck, however there is a method tailor-made for
...
result, model and field data must share the same set of measurement locations,
that is, $X = X^F$. In this way, $X^M$ remains small, including only the unique
...
to a size $m$, as specified by the user. This induces sparsity (i.e, a lot of
zero entries) in the precision matrix (e.g. $\Sigma(X^M)^{-1})$ from Eq.~\ref{eq:mvn_eq}), potentially dramatically speeding up the
evaluation of the likelihood for large values of $n$. The Vecchia approximation
takes an operation of order $O(n^3)$ down to $O(nm^3)$, motivating small values
of $m$.
\begin{align}
...
\citep{katzfuss2021general} have recently made this approximation popular by
working out its representation in matrix form, allowing for broad use in a
variety of different fields, such as spatial statistics and computer
experiments
\citep{katzfuss2020vecchia,katzfuss2021general,zhang2022multi,sauer2023vecchia}.
Logically, as the size of $h(i)$ decreases, the evaluation of the likelihood
speeds up, but the quality of the approximation is reduced. On the flip side,
as one increases $m$, execution time grows, but the quality of the
approximation improves. This continues until $n=m-1$, when the likelihood is no
longer an approximation. For the IBEX simulator data, we find a value of $m <
30$ to be more than sufficient, but the choice can be impacted by the nature of
the data and one's computational budget. The conditioning set can be
constructed in many ways, first by how the data itself is ordered, and second
by how neighbors are selected. Many different orderings have been proposed
\citep{guinness2018permutation}, such as a maxi-min distance strategy. We
default to a random ordering of the training data and nearest-neighbors
approach for selecting the conditioning set, which has been done previously
\citep{sauer2023active} with successful results.

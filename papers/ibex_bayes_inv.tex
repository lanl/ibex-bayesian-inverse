...
data. Typically, a scientist or researcher has some knowledge of the physical
or mathematical model (i.e, the data-generating process), but is seeking to
gain a more in-depth understanding. The goal is often one of two cases: 1)
estimate the parameters (or distribution of the parameters) that govern the
physical model, or 2) if the researcher lacks some knowledge about how the
process was initiated, but knows the true value of model parameters,
reconstruct the inputs from which the observations originated. In this paper,
we are focused on the former, discovering the distribution of unknown
parameters. The motivation for such a method stems from the inability to
either control or precisely measure certain factors (e.g. the force of
gravity) in a physical environment. Therefore, in order to acquire more
insight, scientists develop mathematical models that attempt to represent that
physical process. Often, this results in a sophisticated computer simulation
with an array of input variables. By comparing physical observations to
simulator output generated via unique combinations of model parameters,
statisticians attempt to learn the true, underlying parameter values. A
simple, but classical, approach to this problem is to execute the computer
model a large number of times and find the best match between observed data
and the simulator response via some minimization of error (e.g. least squares,
maximum likelihood).

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.15,trim=0 0 0 70,clip=TRUE]{inv_v_classic.pdf}
...
Bayes' rule, $\pi(\boldsymbol \theta | Y, X) = \mathcal{L}(Y|\boldsymbol
\theta, X) \pi(\boldsymbol \theta) / \pi(Y|X) \propto \mathcal{L}(Y|\boldsymbol \theta,
X) \pi(\boldsymbol \theta)$, begins to illustrate the Bayesian approach to
inverse problems
\citep{kaipio2011bayesian,stuart2010inverse,knapik2011bayesian}. Here $Y$
represents observed output, $X$ denotes a set of controllable, measurable
inputs, and $\boldsymbol \theta$ is a vector of unknown parameters. In
classic Bayes, $\boldsymbol \theta$ contains statistical parameters, say a
mean $\mu$ and variance $\sigma^2$ for Gaussian data, shown in the top half
of Figure \ref{f:inv_v_classic}. In Bayesian Inverse Problems, $\boldsymbol
\theta$ remains important, but the focus is on $\boldsymbol u$
a vector of inputs to a physical or mathematical model or process. As depicted
in Figure \ref{f:inv_v_classic}, parameters $\boldsymbol u$ are passed in to
the model, blackbox or not, which in turn generates output dependent on the
values of the input. This model response forms the parameters $\boldsymbol
\theta$ underlying the data-generating process of $Y$. In both classic Bayes
and Bayesian Inverse Problems, $\boldsymbol \theta$ is used to evaluate the
likelihood $L(Y|\boldsymbol
\theta, X)$. But in Bayesian Inverse Problems, we are only interested in the
distribution of $\boldsymbol u$, pertinent to a simulation or model and
not a direct part of the likelihood calcuation. Instead, the simulator or
model output is used to compute the likelihood of the observed data.

All other aspects of Bayesian inference remains the same. Practioners can
embed a certain amount of subject matter knowledge into the analysis by
through $\pi(\boldsymbol u)$ over model parameters, or rather use an
objective, non-informative prior. Model parameters are treated as random
variables, and therefore have a full joint posterior distribution, instead of
basic point estimates and confidence intervals. In Figure
\ref{f:inv_v_classic} we assume that the field data is Gaussian, a common
approach in most of the literature. In order to perform inference, we must
sample from the posterior of $\boldsymbol u$ via Markov chain Monte Carlo
(MCMC) methods, which we demonstrate next.
...
each location, providing replication. This is common, due to the expense of
changing configurations of a physical experiment, but it need not be the case.
Light gray lines denote individual runs of a computer simulation, at a sample
of different settings of parameters $\boldsymbol{u} = \begin{pmatrix} \mu
& \nu \end{pmatrix}^T$. Finally, the black, bold, dashed line shows the true,
...
estimate the posterior distribution of $\boldsymbol{u}$. Predictions and
...
One difficulty in our situation is the expense of the computer model. Even
with modern-day computing power, some simulations can take hours, days, or
weeks to complete, depending on the a variety of factors, including model
complexity, necessary operations, or the fidelity of the output. This limits
the amount of runs that can be executed and therefore restricts the
exploration of the model parameter space. Therefore, we need a model (what we
call a surrogate, or emulator) fit to the simulator output to make accurate
and reliable predictions for out-of-sample values of our computer model
...
locations. Prhaps the hallmark of GPs is that they naturally provide excellent
uncertainty quantification for out-of-sample predictions, a matter of
importance to scientists exploring these computer models. Therefore, GPs are
widely used in computer experiments and Bayesian Inverse Problems, as
illustrated next.
...
in {\tt Python} \citep{gattiker2020lanl}, is unable to model non-Gaussian

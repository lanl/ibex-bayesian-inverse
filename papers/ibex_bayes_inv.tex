...
$\boldsymbol{u}$.  Given prior information about $\boldsymbol{u}$, we want to
know which $\boldsymbol{u}$'s lead to realizations of $m(\cdot)$ that could
explain $Y$, i.e., the posterior for $\boldsymbol{u} \mid Y$. This is a
classic Bayesian inverse problem
...
$m(\cdot)$, or some statistical parameters $\boldsymbol{\theta} \mid Y$, shown
in the top half of Figure \ref{f:inv_v_classic}. Although
$m(\cdot)=\boldsymbol{\theta}$ remains important, the priority is the
distribution of $\boldsymbol{u} \mid Y$, or the vector of inputs passed into
...
\end{figure}
One drawback to the literature is that modeling is predominately Gaussian (as
depicted in Figure \ref{f:inv_v_classic}), assuming $Y \sim \mathcal{N}\!\left(\mu(\boldsymbol{u}),
\Sigma(\boldsymbol{u})\right)$ as a likelihood, where
$\boldsymbol{\theta}(\boldsymbol{u})=(\mu(\boldsymbol{u}),
\Sigma(\boldsymbol{u}))$ depend on $m(\boldsymbol{u})$. Yet our IBEX
observations are counts. Our first contribution, which is albeit rather
straightforward, is to depart from that literature by perusing a Poisson
likelihood: $Y \sim Pois(\boldsymbol{\lambda}(\boldsymbol{u}))$, where
$\boldsymbol{\lambda}(\boldsymbol{u})$ is determined by $m(\boldsymbol{u})$.
We leave the details of this to Section \ref{sec:gen_bayes_inv}.

Before we go further, let's start with a very simple example that we will
return to in Section \ref{sec:gen_bayes_inv}. Consider the left panel of
Figure \ref{f:toy_calib}. Red stars denote field measurements of some true,
unknown process at different locations $X$, observed with some Gaussian noise.
Additionally, multiple observations are made at each location, providing
replication. This is common, due to the expense of changing configurations of
a physical experiment, but it need not be the case. Light gray lines denote
individual runs of a computer simulation $m(\cdot)$ at a sample of different
settings of parameters $\boldsymbol{u} = \begin{bmatrix} u_1 & u_2
\end{bmatrix}^T$. Finally, the black, bold, dashed line shows the underlying
process $m(\cdot)$ when $\boldsymbol{u}$ is set to the truth.
\begin{figure}[ht!]
\centering
...
To perform inference for $\boldsymbol{u} \mid Y$, we can follow Bayes' rule,
$\pi(\boldsymbol{u}|Y) \propto
\mathcal{L}(Y|\boldsymbol{u})\pi(\boldsymbol{u})$ and utilize
Metropolis-Hastings to sample from the posterior. At each iteration, we
propose a new set of simulation parameters, $\boldsymbol{u}^*$, execute the
computer model $m(\boldsymbol{u}^*)$ to get $\boldsymbol{\theta}$, and then
evaluate the MH-ratio and either accept or reject the proposed values.
Predictions and posterior estimates for our example are shown in the middle
and right panels of Figure \ref{f:toy_calib}. But the hurdle in our situation
we have not addressed with this toy example is the expense of the computer
model. Even with modern-day computing power, some simulations can take hours,
days, or weeks to complete. So, it's not practical to run $m(\cdot)$ inside
the MH sampler. This constrains the amount of runs that can be executed and
therefore restricts the exploration of the model parameter space. Therefore,
we need a model (what we call a surrogate, or emulator) fit to the limited
simulator output to make accurate and reliable predictions for out-of-sample
values of our computer model parameters. Any statistical model can serve as a
surrogate, ranging from linear regression to complex neural networks. However,
the canonical surrogate for computer experiments has come to be the Gaussian
process \citep{gramacy2020surrogates}.

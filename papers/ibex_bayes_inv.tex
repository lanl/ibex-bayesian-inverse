...
{\color{red}(additional visual aids provided in Appendix
\ref{app:resid_anal}).} The local focus of {\sf laGP} limits its ability to
capture global trends, potentially hurting its performance. Large data and
corresponding expensive matrix operations restrict the number of iterations
{\sf deepgp} can run and thus prevents the MCMC from converging to the target
posterior. SEPIA performed similarly to {\color{red}SVEC}, illustrating why it
has had such staying power over the past two decades. As a practitioner, it
appears to be a toss-up between these two methods for which to use. But that's
...
$10^{-4}$) being magnified by the modifications for visual aid listed above.}

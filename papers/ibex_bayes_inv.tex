...
First, ENAs are known to come from sources other than the heliosphere. Space
...
of solving the inverse problem, we update the mean expression in the Poisson
likelihood in Eq.~\ref{eq:pois_inv_bayes_mod} such that $\lambda(X^F) = m(u,
X^F) - \lambda_{b}(X^F)$.
...
($x$, $y$, $z$), time the satellite pointed at that location (in seconds),
counts of ENAs observed at one of six different energy levels (ESA), and a
background rate $\lambda_b$ (ENAs/sec).

Our experimental setup will be similar to that above in Figure
\ref{f:ibex_synth_ex}, only substituting in real data for simulated data. We
execute Algorithm \ref{alg:gibbs} by fitting a Scaled Vecchia GP surrogate to
all $n=66$ computer model runs and sample from the posterior $u \mid Y^F$ via
MCMC. One difference from Section \ref{sec:surr_skymaps} is that field data
($Y^F, X^F$) are not exclusively from one map. Instead, ($Y^F, X^F$) is formed
by stacking the counts, exposure times, background rates, and Cartestian
coordinates $x$, $y$, $z$ from years 2009-2011. Additionally, we only use ENAs
collected at energy level ESA 4. ESA 4 allows the best context for analysis,
whereas lower ESAs have a much weaker signal and higher ESAs overwhelm the
analysis. Raw satellite data from 2009-2011 and our results are shown in Figure
\ref{f:ibex_real_ex}.

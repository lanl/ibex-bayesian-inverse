...
{\color{red}Figure \ref{f:ibex_disc} illustrates the need for more advancements to
properly model disparity between the observed IBEX satellite data and the
corresponding simulator. A fully Bayesian framework could be developed to
provide full uncertainty quantification, perhaps with the use of a deep
Gaussian process to handle the inherent nonstationarity in the discrepancy.}
...
20220107DR. {\color{red}The authors would like to thank C.A.~Eiffer for her
encouragement and support of this paper. They also wish to thank the editor,
...
values for which the colors represent is clipped. These steps are
\textit{only} taken to allow better visual comparison between methods.
...
{\color{red}
Section \ref{sec:emp_comp} assesses the performance of out-of-sample
predictions for the IBEX simulator using our proposed SVEC GP surrogate and
other competitors. In particular, Figure \ref{f:ibex_surr_metrics} displays a
pair of boxplots measuring each surrogate model on RMSE and CRPS. However,
RMSE and CRPS are both univariate metrics and are not able to capture
relationships between output variables (e.g. different pixels). The response
for the IBEX is high-dimensional (16,200), and therefore it is appropriate to
compare using multivariate metrics.}

{\color{red}
First, we consider \textit{energy score} \citep{gneiting2007strictly}, a
natural multivariate extension of CRPS. Energy score incorporates two elements
into the metric: 1) how far away predictions are from the truth, and 2) how
spread out the predictions are from each other. We calculate energy score for
each competitor in Figure \ref{f:ibex_surr_metrics} using draws from the
estimated predictive distribution. Next, we measure the distance between the
predicted response vector $\hat{\lambda}$ and simulator output $\lambda$ using
the infinity norm, $||\hat{\lambda} -\lambda||_\infty$. Figure

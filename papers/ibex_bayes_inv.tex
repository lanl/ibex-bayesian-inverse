...
supporting high-resolution visuals. {\color{red}Therefore, computer model and
field data are potentially collected at different inputs, but we only evaluate
the likelihood (Eq.~\ref{eq:inv_bayes_mod}) at observed field data locations.}
Given prior information about $u$, we want to know which $u$'s lead to
realizations of $m(\cdot, \cdot)$ that could explain $Y^F$, i.e., the
posterior for $u \mid Y^F$.
...
{\color{red}Further visual aids showing the gap in predictive accuracy for
these methods are provided in the Appendix (Figure
\ref{f:surrogate_residuals})}. The local focus of {\sf laGP} limits its
ability to capture global trends, potentially hurting its performance. Large
data and corresponding expensive matrix operations restrict the number of
iterations {\sf deepgp} can run and thus prevents the MCMC from converging to
the target posterior. SEPIA performed similarly to {\color{red}SVEC},
illustrating why it has had such staying power over the past two decades. As a
practitioner, it appears to be a toss-up between these two methods for which
to use. But that's not the entire story.
...
\caption{{\color{red}Absolute residuals averaged over 66 hold-one-out predictions from the IBEX simulator experiment (Figure
\ref{f:ibex_surr_metrics}) for several different surrogate model techniques.
For better visual comparison between methods, the square root of the absolute
residuals is shown and the range of values is clipped. Note that all SVEC and
SEPIA residuals are similar, regardless of the neighborhood size or number of

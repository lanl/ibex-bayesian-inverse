...
without going into much detail. We focus on two particular models available
among many, one for the GDF and one for the ribbon. We leave a more in-depth
physical interpretation and analysis of parameters to more appropriate venues
\citep{zirnstein2021dependence,zirnsteinGDFsims2015}, but to give some context,
we give a simple review here. In the ribbon model, one can think of parallel
mean free path as the distance a hydrogen ion travels beyond the heliopause
before interacting with another particle, receiving an electron, and becoming an
ENA. The ratio parameter is the fraction of parallel mean free path and another
measure of distance, the perpendicular mean free path. The GDF model we use is
constant and is added on to the output of the ribbon simulator.

Real data brings some additional complications, in particular when comparing to
computer models. Scientists know that the makeup of the heliosphere changes over
time, largely due to the variations in the solar wind that accompany the sun's
11 or 21-year cycle. Physicists therefore build in these changes to their
simulators through additional parameters, or restricting the application of
their models to a specific timeframe. This restriction is the case for the GDF
model, which corresponds to the 2009-2011, the first few years of IBEX's
mission. In another approach, the ribbon model is averaged over the entire
sun-cycle. We factor in these heliospheric variations by only comparing
satellite data from the years 2009-2011 to simulator runs.
...
from Section \ref{sec:surr_skymaps} is that field data ($Y^F, X^F$) is not be
...
level ESA 4. ESA 4 allows the best context for analysis, whereas lower ESAs have
a much weaker signal and higher ESAs overwhelm the analysis. Raw satellite data
from 2009-2011 and our results are shown in Figure \ref{f:ibex_real_ex}.
...
These are not as clean as the results we presented in Section
\ref{sec:surr_skymaps}. Our first observation is that the predicted GP surrogate
output at the posterior mean of $u \mid Y^F$ marks a clear difference between
the trends observed in real data. In particular, the intensity of the ribbon
near latitude $-30^\circ$ and longitude $270^\circ$ appears to be much lower
than in the satellite data. There's also a gap, but this time overestimating,
between surrogate and reality between latitudes $60^\circ$ and $120^\circ$.
Next, we can view another indication of this clash in the right panels of Figure
\ref{f:ibex_real_ex}. Note that these both display the same posterior
distribution, but the axis limits are changed. Posterior mass is concentrated in
the corner of the model parameter space. Our priors attempt to push sampling
away from the boundaries, but the likelihood appears to dominate. We take this
as an indication that the computer model is lacking in it's attempt to represent
reality, and the Metropolis sample seeks to go beyond the range of model
parameters available. We address this next.
...
surrogate outputs at the posterior mean our framework estimates for the model
parameter $u$ and data collected from the IBEX satellite. It's clear that the
computer model does not sufficiently account for the behavior observed in the
satellite data. This is not terribly surprising, as simulators are inherently
biased simply from not be able to completely reconstruct the real process.
\citet{kennedyohagan2001} address this limitation by introducing a discrepany
term in canonical computer model calibration framework, a practice followed by
other practitioners
...
IBEX, a point we leave to future work (\ref{sec:discuss}). However, we test the addition of a simple, multiplicative discrepancy term to our original model, modifying Eq.~(\ref{eq:pois_inv_bayes_mod}) as follows:
\begin{align}
Y^F &\sim \mathrm{Pois}(\lambda(X^F), e(X^F)) & \lambda(X) &= m(u, X) \nonumber
...
$\delta$. Mass is concentrated just below 1, indicating no nearly effect from
the multiplicative discrepancy. Results displayed in Figures
\ref{f:ibex_real_ex}-\ref{f:ibex_disc} actually include posterior sampling of
$\delta$ via MCMC. Examining the disparities shown in Figure \ref{f:ibex_disc}
more closely, it appears that there is some spatial dependence in whether the
simulator overestimates or underestimates the mean surface. For instance, the
area of the \textit{sky map} corresponding to the \textit{ribbon} shows some
underestimation. On the flip side, areas around the ribbon are overestimated.
Therefore, a constant multiplicative scale discrepancy is insufficient. In fact,
we believe that $\delta$ is estimated around 1.0 because of these competing
interests between ribbon and GDF. Future work is needed to address thie
potential nonstationarity in the discrepancy surface.
...
extensive, limiting the number of runs available for analysis, demanding a GP
surrogate, but 2) the computer model output is extremely high-dimensional,
requiring unconventional methods to approximate the GP, and 3) data from a field
experiment are Poisson counts, bucking the current literature that focuses
normally distributed observations. Our proposed Poisson Bayesian inverse problem
framework is able to recover the model parameter $u^*$ at the nominal 95\% rate
when provided with synthetic data where the truth is known. We applied this
methodology to data collected from the IBEX satellite and corresponding
simulated heliospheric sky maps from a computer model. When faced with data
collected from the satellite, our method shows that complex discrepancies exist
between the simulator and the physical process it attempts to represent and are
unable to be resolved with a simple multiplicative constant. More advancementare
needed to properly model this parity. Further collaboration with theoretical
astrophysicists responsible for computer model development could also be pursued
to improve the simulator's fidelity to the truth.

Our framework is only applied to counts assumed to come from a Poisson
distributed process. We encourage other practitioners to use this work in other
...
process to handle the inherent nonstationarity in the response surface.

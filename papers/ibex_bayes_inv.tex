...
{\color{red}(in our motivating application: $n_F > 23,000$, $p_F=3$, and $p_U
= 2$)}. These {\color{red}$X$ locations} may be those {\color{red} $X^F \in
...
{\color{red}$X^M \in \mathbb{R}^{n_M \times (p_U+p_F)}$} from a campaign of runs of
$m(\cdot, \cdot)$, {\color{red}where $X$} may or may not coincide with
...
Two such runs, i.e., for two rows $u^\top \in U$ and a dense grid $X$, are
shown in Figure \ref{f:fig1}.
...
applications. {\color{red}However, there is a method tailor-made for our
Bayesian inverse problem setting when the likelihood for $Y^F$ is Gaussian.
We note, however, that it is still insufficient for IBEX, where the likelihood
for $Y^F$ is Poisson.}
...
experiment, we fit {\color{red}an SVEC} GP surrogate on 65 of the $n=66$ model
...
pixels within the same image). The response for the IBEX is high-dimensional

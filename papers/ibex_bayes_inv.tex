...
of observations $Y^M$ at input locations $X^M$. Here we use superscript $M$
to make explicit that our GP surrogate is modeling the simulator
output, not field data. In our situation, the expensive computer model
$m(\cdot,\cdot)$ is run over a small design of space-filling $u$-values ($U$)
paired via Cartesian product with $X_F$ from the field experiment, or a predictive grid $X$,
e.g., $X^M = X^F \otimes U$.   Two such runs, i.e., for two setings $u^\top \in U$  
and using a dense grid $X$,   are shown in Figure
\ref{f:fig1}. 

Evaluating the likelihood $Y_F$ in Eq. \ref{eq:inv_bayes_mod}, e.g., via
Metroplis, may proceed via GP surrogate when {\em in situ} evaluation is too
slow using a corpus of runs $(Y^M, X^M)$ as follows. Suppose $Y^M \sim
\mathcal{N}_{n_m}(\mu(X^M), \Sigma(X^M))$, where $\mathcal{N}_{n_M}$ indicates
an $n_M$-variate MVN. Then, predicting at a new location $x$ (think proposed
$u$-value and paired $X^F$) follows $Y(x) \mid Y(X^M) \sim
...
optimization \citep{jones1998efficient}. %$ , and machine learning \citep{rasmussen2003gaussian}.
...
key insight involved treating $X^F$ (or grid $X$) -- i.e., the fixed reference
set that a simulator realizaition is evaluated on for each candidate parameter
$u$ -- of the {\em output} instead thinking of it as an input. In this way,
...
or $X$, they saught a reduction dimensionality via principal components
...
k_2, \ldots , k_{n_k}$, each scaled by a corresponding weight $w_i(u)$. In this
manner, the high-dimensional response is approximated as
\begin{equation}
...
\label{eq:higdon_pca}
\end{equation}
Weights $w_i(u)$ areeach realizations of a zero-mean GP, fit over principal
components from the limited simulator output. By not directly modeling the
high-dimensional response $m(\cdot,\cdot)$ and it's associated dense grid $X$,
the decomposition of $\Sigma_{w_i}$ avoids the cubic bottleneck in computation
when scaling to large $n$ and can be completed in a reasonable amount of time.
With fitted surrogate in hand, Bayesian inference via MCMC is used to obtain
the posterior distribution of component weights $w_i(u)$. Out-of-sample
predictions at each iteration are obtained by combining predicted weights
$\hat{w_i}(u^*)$ with the calculated basis functions $k_i$. Dimension of the
covariance matrices $\Sigma(X^M)$ and $\Sigma^*(x)$ in Eq.~(\ref{eq:mvn_eq})
remains small and decomposition proceeds quickly.
...
only one library implemenation, originally written in {\tt MATLAB} and now
deployed in {\tt Python} \citep{gattiker2020lanl},  We found it difficult to
...
technology, necessitating a more complex approach. It is not longer necessary
to play the trick of moving reference grid to the output space.  A GP
...
\section{Generalized Bayesian inverse problems} % with GP surrogates}
...
\includegraphics[scale=0.75,trim=35 0 20 0,clip=TRUE]{ibex_sim_est.pdf}

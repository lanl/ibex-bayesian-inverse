...
computer models. Scientists know that the makeup of the heliosphere changes
over time, largely due to the variations in the solar wind that accompany the
sun's 11 or 21-year cycle. Physicists therefore build in these changes to their
simulators either through additional parameters, or restricting the application
of their models to a specific timeframe. This restriction approach is the case
for the GDF model, which corresponds to the 2009-2011, the first few years of
IBEX's mission. In another approach, the ribbon model is averaged over the
entire cycle of the sun. We factor in these heliospheric variations by only
comparing satellite data from the years 2009-2011 to simulator runs.
...
Our first observation is that the predicted GP surrogate output at the
posterior mean of $u \mid Y^F$ marks a clear difference between the trends
observed in real data. In particular, the intensity of the ribbon near latitude
$-30^\circ$ and longitude $270^\circ$ appears to be much lower than what is
observed in the satellite data for each year. There's also a gap (this time
overestimating) between surrogate and reality between longitudes $60^\circ$ and
$120^\circ$. We view another indication of this clash in the right panels of
...
corner of the model parameter space. Our priors attempt to push sampling away
from the boundaries, but the likelihood appears to dominate. We take this as an
indication that the computer model is lacking in it's attempt to represent
reality, and the Metropolis sampler seeks to go beyond the range of model
...
surrogate outputs at the posterior mean estimated by our framework for the
model parameter $u$ and data collected from the IBEX satellite. It's clear that
the computer model does not sufficiently account for the behavior observed in
the satellite data. This is not terribly surprising. Simulators are inherently
biased, simply from not being able to completely reconstruct the real process.
...
the multiplicative discrepancy. In fact, our results for the real IBEX data
displayed in Figures \ref{f:ibex_real_ex}-\ref{f:ibex_disc} actually include
posterior sampling of $\delta$ via MCMC. Examining the disparities shown in
Figure \ref{f:ibex_disc} more closely, it appears that there is some spatial
...
\textit{ribbon} shows some underestimation. On the flip side, areas around the
ribbon are overestimated. Therefore, a constant multiplicative scale
discrepancy is insufficient. In fact, we believe that $\delta$ is estimated
around 1.0 because of these competing interests between ribbon and GDF. Future
work is needed to address thie potential nonstationarity in the discrepancy
surface.

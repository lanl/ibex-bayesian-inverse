...
insight is treating the dense grid $X$ on which a simulator is evaluated as part
of the output instead of an input. In this way, $X^M$ remains small, including
only the unique combinations of unknown parameters $u$. Furthermore, the
dimensionality of the computer response itself can be reduced by employing
principal components analysis (PCA), representing the vector output of a
simulator model $m(\cdot,\cdot)$ as the sum of $n_k$ basis functions $k_1, k_2,
\ldots , k_{n_k}$, each scaled by a corresponding weight $w_i(u)$. In this
manner, the high-dimensional response is approximated as
\begin{align}
m(u, X) &\approx \sum_{i=1}^{n_k}  k_i \cdot w_i (u), \quad n_k \ll n_X \ \nonumber
\\ w_i(u) &\sim \mathcal{N}( 0, \Sigma_{w_i}).
\label{eq:higdon_pca}
\end{align}
Weights $w_i(u)$ are realizations of a zero-mean GP, fit over principal
components from the limited simulator output. Therefore, by not directly
modeling the high-dimensional response $m(\cdot,\cdot)$ and it's associated
dense grid $X$, the decomposition of $\Sigma_{w_i}$ avoids the cubic bottleneck
in computation when scaling to large $n$ and can be completed in a reasonable
amount of time. With fitted surrogate in hand, Bayesian inference via MCMC is
used to obtain the posterior distribution of component weights $w_i(u)$.
Out-of-sample predictions at each iteration are obtained by combining predicted
weights $\hat{w_i}(u^*)$ with the calculated basis functions $k_i$. As a result
of this approximation, the dimension of the covariance matrices $\Sigma(X^M)$
and $\Sigma^*(x)$ in Eq.~\ref{eq:mvn_eq} remains small and decomposition
proceeds quickly.
...
their method and implementation, originally written in {\tt MATLAB} and now
deployed in {\tt Python} \citep{gattiker2020lanl}, is unable to model a

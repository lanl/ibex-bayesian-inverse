...
\citep{kaipio2011bayesian,stuart2010inverse,knapik2011bayesian}. To be clear,
we are not directly interested in the output of our computer model,
$m(\cdot)$, or some statistical parameters $\boldsymbol{\theta}$, as shown in
the top half of Figure \ref{f:inv_v_classic}. Although
$m(\cdot)=\boldsymbol{\theta}$ remains important, our focus is on the
distribution of $\boldsymbol{u} \mid Y^F$, or the vector of inputs passed into
the physical or mathematical model.
\begin{figure}[ht!]
\centering
...
and the approach to Bayesian Inverse Problems (bottom). Output $Y^F$ is Gaussian in
either case, but parameters $\boldsymbol{\theta}$ differ in where they originate.
Note that the ``example'' for Bayesian Inverse Problems is also comprehensive, as it
is the only context addressed in the current literature.
\label{f:inv_v_classic}}
\end{figure}

We start with a very simple example that we will return to in Section
\ref{sec:gen_bayes_inv}. Consider the left panel of Figure \ref{f:toy_calib}. Red
stars denote field measurements $\left(Y^F\right)$ of some true, unknown process at different
field locations $X^F$, observed with Gaussian noise. Multiple
measurements are made at each location, providing replication. This is common, due to
the expense of changing configurations of a physical experiment, but it need not be
the case. Light gray lines indicate individual runs of a computer simulation
$m(\cdot)$ at a sample of different settings of parameters $\boldsymbol{u} =
\begin{bmatrix} u_1 & u_2
\end{bmatrix}^T$. The black, bold, dashed line shows the underlying process
...
computer model. Furthermore, in our realm of high-dimensional responses, $X^F$ simply
...
Metropolis-Hastings (MH) to sample from the posterior. At each iteration $t$, we
propose a new set of simulation parameters $\boldsymbol{u}^*$ and execute the
computer model $\left(m(\boldsymbol{u}^*)\right)$ to get $\boldsymbol{\theta^*}$.
Then, we evaluate the MH-ratio and either accept or reject the proposed values.
Predictions and posterior estimates for our example are shown in the middle and right
panels of Figure \ref{f:toy_calib}. Note that each of the light gray lines in the
middle panel represents a single run of the simulation at one of the accepted set of
model parameters, $\boldsymbol{u}^{(t)}$. That is, each gray line corresponds to a
circle in the right panel, where posterior draws of $\boldsymbol{u}$ are displayed.
It is clearly evident that the MH sampler thoroughly explores the input space and
that it successfully captures the true value of $\boldsymbol{u}$.
...
\Sigma\left(X^F\right))$ depends on $m(\boldsymbol{u})$. Yet our IBEX
observations are counts. Our first contribution, which is albeit rather
straightforward, is to depart from that literature by perusing a Poisson likelihood:
$Y^F \sim Pois(\boldsymbol{\lambda}(\boldsymbol{u}))$, where
$\boldsymbol{\lambda}(\boldsymbol{u})$ is determined by $m(\boldsymbol{u})$. We leave
the details of this to Section \ref{sec:gen_bayes_inv}.

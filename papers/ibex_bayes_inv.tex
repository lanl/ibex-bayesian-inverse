...
$\boldsymbol \lambda = f(X)$, as shown in Figure 3.
 % We know that the
% underlying mean process is of the form λi = µ+ νf(xi). We are unable to modify
% µ and ν in a physical setting, but we can vary these values in a trivial,
% one-line “computer model.” If we consider all unique combinations for the
% values of µ ∈ {5,7.5,10,12.5,15}and ν ∈{0.25,0.625,1,1.375,1.75}, we end up
% with 25 different functional evaluations (depicted as gray lines in 3). Our
% goal is two-fold: 1) Make accurate predictions at new, unobserved physical
% locations (x′
% i) with good uncertainty quantification (UQ), and 2) Estimate the distribution of calibration
% parameters µ and ν to better understand how the mean process is constructed.
% Consider the output displayed in Figure 3. Suppose we are given counts collected from a physical
% experiment at different input locations (xi). We know that the underlying mean process is of the
% form λi = f(xi,µ,ν). We are unable to modify µand ν in a physical setting, but we can vary thes{}e
% values in a computer model. We run the simulation for a variety of unique combinations for the
% 11values of µand ν, resulting in 25 different functional evaluations (depicted as gray lines in 3). Our
% goal is two-fold: 1) Make accurate predictions at new, unobserved physical locations (x′
% i) with good
% uncertainty quantification (UQ), and 2) Estimate the distribution of calibration parameters µ and
% ν to better understand how the mean process is constructed.

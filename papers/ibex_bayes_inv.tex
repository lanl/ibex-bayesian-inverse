...
For simplicity, we choose $u^*$ such that it falls in the middle of the
parameter space. Next, we gather information from real data corresponding to a
single year that the IBEX satellite has been in operation. We only use the
measurement locations and associated exposure times and background rates and
ignore the observed counts of ENAs. Then, for each latitude/longitude pair
(i.e.~a row in $X^F$), we draw synthetic satellite (or field) data from a
Poisson, $Y_i^F \sim \mathrm{Pois}(\lambda^*_i, e_i)$, where $\lambda_i^*$ is
determined by the computer model $m(\cdot,\cdot)$. The center panel of Figure
\ref{f:ibex_synth_ex} shows the result, which we will regard as data we
observed. Note that observations are not on a fine grid, but instead match the
recorded coordinates for ENAs captured by the IBEX satellite in it's orbit.
...
(HPD) interval. The posterior mean of $u$ lies at \textit{parallel mean free
path} $=1675.61$ and \textit{ratio} $=0.0178$, close to $u^*$, demonstrating
that our framework is able to concentrate mass around the true model parameter.
As a sanity check, we take the value of the estimated parameter at the
posterior mean and plug it in to our fitted Scaled Vecchia GP surrogate. The
predicted output is shown in the bottom left panel of Figure
\ref{f:ibex_synth_ex}. There are some subtle differences between the estimated
sky map and $\lambda^*$ (the ``truth'') in the top right panel (i.e, the shape
and intensity of the ribbon vary slightly between the two). However, each map
could have reasonably generated the data we ``observe'' in the top left panel.
...
values of ratio. We note that the spread of parameter values was determined by
space scientists to include all values considered reasonable: parallel mean
free path ranges from 500-3000 astronomical units (AU); ratio values are
unitless, and span from 0.001 to 0.1. Due to the inability for the model to
furnish posterior samples beyond the boundary parameters, we do not test those
combinations for which at least one parameter is on its edge (i.e, when
parallel mean free path $\in \{500, 3000\}$ and/or ratio $\in \{0.001, 0.1\}$).
Removing these edge cases results in 36 remaining test runs. For each
combination, we follow the same procedure as above: select one model parameter
$u$ and create simulated observed satellite data, fit a  Scaled Vecchia
surrogate on training data excluding $u^*$, and run Algorithm \ref{alg:gibbs}
to sample from the posterior $u \mid Y^F$. Results are shown in Figure
...
synthetic satellite data. Our next task is to apply this to real data, with the
goal of helping scientists better understand the behavior of the heliosphere by
...
% for different model parameters, a simulation can be run to output a
% proposed % \textit{sky map}. As stated previously, Bayesian Inverse problems
% are % concerned with leveraging simulator output for the purpose of
% estimating the % distribution of model parameters. This is the primary
% interest of physicists % involved in this project, as estimation of these
% parameters will give credence % to what model best represents the data.

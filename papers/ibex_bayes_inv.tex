...
rate at which these particles are created throughout the heliopause. An example
...
the heliosphere's creation and evolution.
...
ribbon-only model developed by \citet{huang2025numerical}. These simulators take
inputs that can be varied to modify the shape and intensity of the ribbon. The
ribbon-only model relies on two parameters, \textit{parallel mean free path} and
\textit{ratio}, while the GDF model we use remains constant and is simply added
on to the output of the ribbon model. Dialing in good settings for these input
parameters, in light of observed ENA counts from IBEX, is instrumental in
furthering theory development and validation.
...
requiring thousands of iterations to converge.  While it's rather

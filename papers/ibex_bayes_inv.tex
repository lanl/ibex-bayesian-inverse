...
that have caused space scients to revise hypotheses about the physical
processes, and computrer simulations under those models, in play at the
...
thwarting convetional methods; and (3) IBEX observations are counts, whereas
...
which compare favorably to to alternatives, through multiple simulated
...
take thousands of core hours.  This severely limit the number of unique 
...
data IBEX provides, and along with computer simulated rates, present some
...
counts).  It is worth noting that Bayesian posterior sampling, say via via
Markov chain Monte Carlo (MCMC), compounds computational bottlenecks (2).

Methods exist separately, in the literature, to address needs each of the
aforementioned situations 1)--3), but we are not aware of anything in the
intersection. We propose a framework for Bayesian inverse problems that
utilizes the Vecchia approximation
\citep{katzfuss2021general,scaledvecchiakatzfuss2022} with a Gaussian process
surrogate or emulator for high dimensional computer simulations, and couple
that with a Poisson observational model to furnish posterior samples of
unknown parameters to these models given observed data.

%% OTHER WORK / CHALLENGES
Table \ref{tab:prev_work} summarizes the features of the IBEX inverse problem
and reviews the necessary capabilities, alongside recent papers offering
partial solutions.  References provided are representative.  We acknowledge
that there are, in most cases, several works that may fit the bill.  Each
row in the table may be summarized as follows.
...
{\em Steven, you still don't say what you're contribution is before the
roadmap.}

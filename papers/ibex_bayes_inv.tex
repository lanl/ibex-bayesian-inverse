...
normally distributed observations. In simulated experiments, our Scaled Vecchia
GP surrogate provides accurate predictions at input locations not included in
training data. Our Poisson Bayesian inverse problem framework is able to recover
the true model parameters at the nominal 95\% rate when provided with synthetic
data where the model parameter $u$ is known. We applied this methodology to data
collected from the IBEX satellite, and corresponding simulator output generating
heliospheric sky maps. When faced with real data from the satellite, our method
shows that discrepancies exist between the computer model and the physical
process it attempts to represent. Further collaboration is needed to inform the
theoretical astrophysicists responsible for model development where the model is
lacking, and improve its fidelity to the truth.
...
contexts where data from the field may come from some other distribution (e.g.
Negative Binomial, Bernoulli, Exponential, etc.). The Scaled Vecchia GP
surrogate used for the computer model also fails to provide full uncertainty
quantification on its parameter estimates. A fully Bayesian framework could be
developed, perhaps with the use of a deep Gaussian process to handle the
inherent nonstationarity in the response surface.

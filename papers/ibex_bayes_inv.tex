...
$u$ of a computer model that produce a (simulated) sky map that is most likely
to have generated some observed.

Before using the actual ENAs from the satellite, we run our Poisson Bayesian
inverse problem framework on synthetic satellite data where we know the
``true'' parameter $u^*$ and can evaluate whether our method is able to recover
it.

We aim to pair the observed satellite data with the $n=66$
computer model runs to learn the distribution of the IBEX ribbon simulator
parameters \textit{parallel mean free path} and \textit{ratio}.

Before diving in, a couple details about the data collected by the IBEX satellite and the implementation of the computer model are necessary to explain.

First, ENAs are known to come from sources other than the heliosphere, so space
scientists quantify this noise and add it as a background rate ($\lambda_b$)
that must be accounted for. Although this rate is an estimate rather than an
exact value, we follow the lead of other researchers \citep{osthustheseus2023}
and treat it as constant in our work. For example, the left panel of Figure
\ref{f:fig1} displays ENA rates {\em after} $\lambda_b$ has been substracted
out. For the purpose of solving the inverse problem, the mean in the Poisson
...
without going into much detail. We leave a more in-depth physical
interpretation and analysis of parameters to more appropriate venues
\citep{zirnstein2021dependence,zirnsteinGDFsims2015}, but to give some context,
we give a simple review here. We focus on two particular models available among
many, one for the GDF and one for the ribbon. In the ribbon model, one can
think of the parameter \textit{parallel mean free path} as the distance a
...
measure of distance, the perpendicular mean free path. The GDF model we use is
constant and is simply added on to the output of the ribbon simulator.

Scientists know that the makeup of the helioshpere changes over time, largely
due to the variations in the solar wind that accompany the sun's 11 or 21-year
cycle. Physicists therefore build in this evolution to their simulators either
through additional paramters, restricting the application of their models to a
specific timeframe, or averaging over the entire cycle of the sun. The GDF
model we work with corresponds to the years 2009-2011, while the ribbon model
is averaged over the entire sun cycle. We factor in these heliospheric
variations by only comparing satellite data from the years 2009-2011 to
simulator runs.
...
single year that the IBEX satellite has been in operation, ignoring the
observed counts of ENAs. For each latitude/longitude pair (i.e.~a row in
$X^F$), we draw synthetic satellite (or field) data from a Poisson, $Y_i^F \sim
...
true model parameter. As a sanity check, we take posterior mean and plug it in
to our fitted Scaled Vecchia GP surrogate. The predicted output is shown in the

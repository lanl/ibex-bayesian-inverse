...
$m(\cdot, \cdot)$ that may or may not coincide with settings in $X^M$. We
prefer a small design of space-filling $u$-values ($U$), paired via Cartesian
product with $X_F$ from the field experiment, or a predictive grid $X$, e.g.,
$X^M = X^F \otimes U$, depending on the situation.   Two such runs, i.e., for
...
to in order to extend it to new inputs.  Many surrogates for computer
...
a Bayesian MCMC setting.  For IBEX, the output ``images'' are massive ($\gg
15{,}000$ pixels) for each $u$-value, a non-started without adjustment.  Most
of our modeling effort involves circumventing this bottleneck, however there
is a method tailor-made for this setting when the likelihood (for $Y_F$) is
Gaussian.

%------------------------------------------------------------------------------
%% SECTION 2.2 - Principal Components for High-Dimensional Response
\subsection{Principal components approximation}
\label{sec:pca}
%------------------------------------------------------------------------------
...
\section{Large scale, generalized Bayesian inverse problems} % with GP surrogates}

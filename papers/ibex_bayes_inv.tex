...
The boundary-pushing estimate for $u$ and the plain visual gap between surrogate
and collected data are somewhat unsatistfying. We address the potential
discrepancy between simulator and reality in more detail in Section
\ref{sec:discuss}. But we wish to offer some validation that our method performs
well for the real data, given our provided computer model implementation. To do
so, we run a final experiment using CRPS and cross validation (Figure
\ref{f:real_cv_valid}). We split the satellite data from 2009-2011 into ten
folds. For each fold $f$, we run our Poisson Bayesian inverse problem framework,
only using the other nine folds as the field data, and store the resulting model
...
posterior mean; 2) over a fine grid of ratio, with parallel mean free path fixed
...
Figure \ref{f:real_cv_valid}. First and foremost, our previous results using all
the data provide an estimate of $u$ that resides in the region with the best
performing values of $u$ in terms of CRPS. Next, it's obvious that the value of
ratio largely determines how well the surrogate predictions fit the real data.
Looking back at Figure \ref{f:ibex_real_ex}, it's interesting to note that the
posterior is spread across a wide range of values for parallel mean free path
($\sim$2700-3000). That range corresponds to some of the lowest values of CRPS.
Lastly, there appears to be some interaction between the two parameters.
Changing parallel mean free path which ratio $= 0.1$ greatly affects CRPS.
However, when ratio $= 0.001$, varying parallel mean free path doesn't change
results.

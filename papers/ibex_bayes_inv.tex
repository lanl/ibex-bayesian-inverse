...
\href{mailto:sdbarnett@lanl.gov}{\tt sdbarnett@lanl.gov}, Department of
Statistics, Virginia Tech; Statistics Group, Los Alamos National Laboratory}
\and Robert B. Gramacy\thanks{Department of Statistics, Virginia Tech} \and
Lauren J. Beesley\thanks{Statistics Group, Los Alamos National Laboratory}
\and Dave Osthus\footnotemark[3] \and Yifan Huang\thanks{ Center for Space
Plasma and Aeronomic Research, University of Alabama in Huntsville }
...
{\color{red} in terms of accurate out-of-sample prediction accuracy and our
ability to recover the true, ``unknown'' computer model parameters.} We then
apply this new technology to IBEX satellite data and associated computer
...
chain Monte Carlo (MCMC), compounds {\color{red} computational challenges} 2).
We propose a framework for Bayesian inverse problems that utilizes the Vecchia
approximation \citep{katzfuss2021general,scaledvecchiakatzfuss2022} with a
Gaussian process (GP) surrogate or emulator for high-dimensional computer
simulations, and couple that with a Poisson observational model to furnish
posterior samples of unknown parameters to these models given observed data.
...
We study IBEX through the lens of observational data {\color{red}$Y^F \in
\mathbb{R}^{n_F}$} ($F$ for field measurement) and a computer
implementation of a physical model $m(u, X) = \{m(u, x_i^\top)\}_{i=1}^n$.
Parameters {\color{red}$u
\in \mathbb{R}^{p_U}$} are unknown and {\color{red} $X \in \mathbb{R}^{n
\times p_F}$} represents the {\color{red} locations (i.e., spatial coordinates
in columns $x$, $y$, $z$)} where the simulation output is evaluated
{\color{red}(in our motivating application: $n_F > 23,000$, $n = 16,200$,
$p_F=3$, and $p_U = 2$)}. These {\color{red}$X$ locations} may be those
{\color{red} $X^F \in
\mathbb{R}^{n_F \times p_F}$ } that correspond to field measurements $Y^F$, or
a grid of predictive locations supporting high-resolution visuals.
{\color{red}Therefore, computer model and field data are potentially collected
at different inputs, but we only evaluate the likelihood (provided momentarily
as Eq.~\ref{eq:inv_bayes_mod}) at observed field data locations.} Given prior
information about $u$, we want to know which $u$'s lead to realizations of
$m(\cdot, \cdot)$ that could explain $Y^F$, i.e., the posterior for $u \mid
Y^F$.

This is a classic Bayesian inverse problem
\citep{kaipio2011bayesian,stuart2010inverse,knapik2011bayesian}.
Eq.~\ref{eq:inv_bayes_mod} provides the typical hierarchical model used in
this setting, {\color{red} modeling} field data $Y^F$ {\color{red}as
a $n_F$-variate Gaussian.} % with defined mean and covariance functions.}
\begin{align}
Y^F &\sim \mathcal{N}_{n_F}\!\left(\mu\!\left(X^F\right),
\Sigma\!\left(X^F\right)\right) \nonumber  \\
\mu(X) &= m(u, X) \label{eq:inv_bayes_mod} \\
\Sigma(X) &\sim \pi\left(\Sigma\right) & \mbox{e.g.,} \quad \Sigma^{-1} & \sim \mathrm{Wishart}(\cdot, \cdot)
\nonumber \\
u &\sim \pi(u) & \mbox{e.g.,} \quad u &\sim \mathrm{Unif}[0,1]^{p_U} \nonumber
\end{align}
...
different $u$ on a fine grid of spatial locations $X$. {\color{red}(The number
of unique runs, $n_U$, in this toy example totals 30).} The bold-black-dashed
...
\caption{{\color{red}A simple inverse problem. Observations ($Y^F$, measured
at $X^F$) from the field and a sample of computer model runs (each evaluated
over a fine grid $X$) are displayed in the left column. In the top row, red stars
are measurements with Gaussian noise. On the bottom, red stars indicate individual
draws from a Poisson that, when summed together, represent $Y^F$ in
Alg.~\ref{alg:gibbs}. Red circles correspond to the quantity
$\frac{Y^F}{e(X^F)}$. Middle panels show computer model evaluations at
posterior draws and means. The right column shows posterior distributions of
model parameter $u$.}
\label{f:toy_calib}}
\end{figure}

The top middle panel of the figure shows runs of our virtual model evaluated
at posterior draws {\color{red}$u^{(t)}$ ($t$ indicating the $t^\mathrm{th}$
step of the sampler)} obtained under Metropolis sampling using a multivariate
normal likelihood whose mean $\mu(X^F)=m(u^{(t)}, X^F)$ and whose covariance
$\Sigma(X^F) \sim
...
are shown in the top right panel of the figure.  In this toy example, it is
...
predominantly in this way, particularly {\color{red}regarding} a Gaussian
likelihood. We aim to provide a method that achieves the same goal, but for
unknown parameters $u$ that underlie observed IBEX satellite data (left panel
of Figure \ref{f:fig1}), whose observations are counts under variable
exposure. Our first contribution, which is albeit rather straightforward, is
to depart from that literature by using a Poisson likelihood.
...
\label{eq:pois_inv_bayes_mod} \\ u &\sim \pi(u) & \mbox{e.g.,} \quad u &\sim \mathrm{Unif}[0,1]^{p_U},
\end{align}
...
right panels of Figure \ref{f:toy_calib}. Similar to results in {\color{red}
the top row}, it is clear that the sampler searches a broad swath of the model
input space but hones in {\color{red}(perhaps more effectively than before)}
on the region where the true parameter value lies. {\color{red}In the bottom
middle panel, we do not expect the posterior draws to cover all observed
counts, but to be centered around the true mean, as is the case here.}
...
slow. Suppose we have a vector of observations {\color{red} $Y^M \in
\mathbb{R}^{n_M}$ } ($M$ for computer model) at input locations
{\color{red}$X^M \in \mathbb{R}^{n_M \times (p_U+p_F)}$} from a campaign of
runs of $m(\cdot, \cdot)$, {\color{red}where $X$} may or may not coincide with
settings in $X^F$. We prefer a small design of space-filling $u$-values {\color{red}($U
\in \mathbb{R}^{n_U \times p_U}$)}, paired via Cartesian product with $X^F$ from
the field experiment or a predictive grid $X$, e.g., $X^M = X \otimes U$,
depending on the situation. Two such runs, i.e., for two rows $u^\top \in U$
and a dense grid $X$, are shown in Figure \ref{f:fig1}.
...
evaluations {\color{red}($n_U < 50$)} are shown. For these toy examples, we
...
cubic in $n_M$, respectively.  This limits $n_M$ to a few thousand at most in
a Bayesian MCMC setting, necessitating approximations for GPs that curb this
prohibitive cost. With the 1d example in Figure \ref{f:toy_calib}, we have
{\color{red}$n_M=n \times n_U = 20 \times 30$}, combining $X$ and $U$, which
is manageable. But for IBEX, the simulated sky maps like those in the middle
and right panels of Figure \ref{f:fig1} are in 2d (under the hood the output
is in spherical coordinates $x$, $y$, and $z$) and results in $n >16{,}000$
locations (or pixels) for each $u$-value. Combine that with
{\color{red}$n_U=66$} unique $u$-values used in runs available to us and our
response vector will have greater than one million elements. Even if we used a
smaller LHS space-filling design of size {\color{red}$n_U=15$} over the model
...
Most of our modeling effort in this paper involves circumventing the
$O(n_M^3)$ computational bottleneck associated with GPs. The prevalence of
methods addressing this issue has grown significantly in the past decade as
increased access to computing resources leads to problems with larger and
larger training datasets. These methods include, but are not limited to,
fixed-rank kriging \citep{cressie2008fixed}, inducing points
...
nearest neighbors \citep{datta2016hierarchical, wu2022variational}. Most of
these are general enough to be used in a variety of different fields and
applications.

For Gaussian inverse problems, however, \citet{higdoncalib2008} developed a
method tailored to settings with expensive simulations and high-dimensional
outputs, such as images, shape descriptions, and time series. {\color{red}They faced
computational bottlenecks similar to those posed by the IBEX simulator: the
expense of running the simulator limits the number of evaluations available
for analysis, while each run generates a high-dimensional response. Despite
this similarity, their method assumes a Gaussian likelihood for the field
observations $Y^F$ and therefore cannot be applied directly to IBEX, where the
likelihood for $Y^F$ is Poisson.}

\citeauthor{higdoncalib2008}'s key insight involved treating $X^F$ (or gridded
$X$) -- i.e., the fixed reference set that a simulator realization is
evaluated on for each candidate parameter $u$ -- as a feature of {\em output},
rather than conventionally as an input. As a result, model and field data
share the same set of measurement locations, that is, $X = X^F$. In this way,
$X^M=U$ and remains small, including only the unique combinations of unknown
parameters $u$. Then, rather than keep track of all of those outputs $Y^M$,
indexed over $X^F$ or $X$, dimensionality may be dramatically reduced via
principal components analysis (PCA). Specifically, they proposed representing
the vector output of a simulator model $m(\cdot,\cdot)$ as the sum of $n_k$
basis functions $k_1, k_2,\ldots , k_{n_k}$, each scaled by a corresponding
weight $w_i(u)$: $m(u, X) \approx \sum_{i=1}^{n_k}  k_i \cdot w_i (u)$, where
$w_i(u) \sim \mathcal{N}( 0,
...
We begin evaluation of {\color{red}SVEC's} performance on the IBEX simulator
with a visual assessment. Scientists at LANL have provided us with $n_U=66$
unique model runs, each producing an image of 16,200 points, or pixels. Figure
\ref{f:ibex_sim_surr} shows four of the actual simulation outputs (corner
panels displayed with thin, solid borders). {\color{red}An SVEC} GP surrogate
is fit to all {\color{red}$n_U=66$} simulated runs. Then, we predict the
output for five unobserved combinations of the model parameters that fall in
between the actual runs shown. These are depicted in the cross of Figure
\ref{f:ibex_sim_surr}, those panels with thicker, dashed borders. To the naked
eye, these predictive surfaces appear to have been generated by the computer
model itself. Moving left to right, or top to bottom, the surfaces change
gradually and as one would expect. Next we show empirically that our surrogate
fit is accurate and outperforms competitors.
...
unobserved input combinations from a GP surrogate fit to $n_U=66$ runs of the
computer model. Model parameter settings for the predictive panels fall
between the four actual simulator runs shown.
...
experiment, we fit {\color{red}an SVEC} GP surrogate on 65 of the
{\color{red}$n_U=66$} model runs available. Then, we predict at the held out
model parameter. For now, just pay attention to the boxplots on the left-hand
side of each panel. We'll come back to the others in a moment. It is obvious
that {\color{red}SVEC} outperforms {\sf laGP} and {\sf deepgp} in both RMSE
and CRPS. {\color{red}Additional visual aids are provided in Appendices
\ref{app:resid_anal} and \ref{app:multivar_mets}.} The local focus of {\sf
laGP} limits its ability to capture global trends, potentially hurting its
performance. Large data and corresponding expensive matrix operations restrict
the number of iterations {\sf deepgp} can run and thus prevents the MCMC from
converging to the target posterior. SEPIA performed similarly to
{\color{red}SVEC}, illustrating why it has had such staying power over the
past two decades. As a practitioner, it appears to be a toss-up between these
two methods for which to use. But that's not the entire story.
...
model available for training. To test the former, we vary the dimensionality
of the response from $n=200$ to $n=20,000$, while keeping the number of runs
constant. For the two methods that are best equipped for large dimensions
({\color{red}SVEC} and SEPIA), we attempt to stretch their capacity by
cranking up the length of the response vector from 20-75K by steps of 5K. At
each dimension size, we run five Monte Carlo iterations for each method and
...
\caption{Timing metrics on IBEX. A comparison of the fastest methods
{\color{red}on larger training/test sets} is shown in the right panel, with
different line types indicating neighborhood size ($m$) and number of
principal components ($n_k$) used for {\color{red}SVEC} and SEPIA,
...
\caption{\color{red}{Timing metrics on IBEX. Left and middle
...
size of the training dataset (i.e., the number of simulator runs
{\color{red}$n_U$}). SEPIA was specifically built for computer experiments
with high-dimensional output. While it clearly performs well in these
situations, it's also fairly inflexible to other contexts, such as a large
number of simulation runs. {\color{red}SVEC} does not face this hurdle, as it
treats both high-dimensional output and large-scale univariate datasets
similarly, as a scalar response. Our experiment is set up similar to the
previous one, but we keep the dimensionality of the response at 10K while
sliding the number of simulator runs from $n_U=10$ to $n_U=100$. Again, all
methods are run over five MC iterations for each value of {\color{red}$n_U$}.
For the higher performing methods, we push the campaign size up further
{\color{red}($n_U=2500$)}.
...
right panel of Figure \ref{f:ibex_surr_timing_size}. For {\color{red}$n_U > 500$}, SEPIA
...
competitor when $m<50$, and for all values of $m$ when {\color{red}$n_U>1500$}.
...
We now return to the IBEX inverse problem. We wish to determine the settings
$u$ of a computer model that produce a sky map most likely to have generated
observed counts and, more specifically, quantify the uncertainty on our
estimates of $u$. Before diving in, we dispense a few important details about
IBEX observations and computer simulations.  We then test our modeling
apparatus on synthetic field observations (computer model at known $u^\star$
and with Poisson counts) to ensure good performance within the model class,
and where we know the true calibration parameter.  Finally, we report on our
calibration of the actual IBEX satellite observations.
...
We have referred to the IBEX simulator throughout this paper without going
into much detail. While we leave a more in-depth physical interpretation and
...
perpendicular mean free path, another measure of distance, and
\textit{parallel mean free path}. These are our $u$ values.
{\color{red}Although seemingly entangled, this parameterization is common
practice among space scientists, easing interpretation and allowing for the
clean separation of certain particle scattering and diffusion effects.} The
response from the GDF model is static and is merely added to the output of the
ribbon simulator. This is not the case for GDF simulators in general, but the
GDF model we have access to does not vary with any parameters of interest.
...
Using the corpus of {\color{red}$n_U=66$} computer model runs, varying a
design of \textit{parallel mean free path} and \textit{ratio} $u$-values,
described in Section \ref{sec:vecchia}, we select a value $u^\star$ and set
the corresponding output ($\lambda^\star = m(u^\star, X)$) as the
{\color{red}``unknown''} underlying mean ENA rate (top middle panel of Figure
\ref{f:ibex_synth_ex}). Next, we gather measurement locations $X^F$, exposure
times $e$, and background rates $\lambda_b$ from a single year that the IBEX
satellite has been in operation, ignoring the observed counts of ENAs in this
fabricated example. For each latitude/longitude pair (i.e.~a row in $X^F$), we
draw synthetic satellite (or field) data from a Poisson, $Y_i^F \sim
...
away 1,000 as burn-in and thinning by 10. Figure \ref{f:ibex_synth_ex}
displays the remaining posterior samples of $u$ (bottom left) as a kernel
density along with a 95\% highest posterior density (HPD) interval. The
posterior mean of $u\mid Y^F$ lies near $u^\star$, demonstrating that our
framework is able to concentrate mass around the true,
{\color{red}``unknown''} model parameter. As a sanity check, we take the
estimated posterior mean and plug it in to our fitted {\color{red}SVEC} GP
surrogate of the IBEX simulator. The predicted output is shown in the
{\color{red}top right} panel of Figure \ref{f:ibex_synth_ex}. There are some
nearly imperceptible differences between the estimated sky map and the true
$\lambda^\star$ (i.e., the shape and intensity of the ribbon vary slightly
between the two). {\color{red}These residuals between simulated and predicted
output (bottom right panel) are orders of magnitude smaller than the response,
but do exhibit structure, which we will address in Section \ref{sec:discuss}}.
However, from a purely visual standpoint, it appears that each map could have
reasonably generated the data we ``observe'' in the top left panel.
...
To further validate our methodology, we conduct this experiment for each
unique model parameter available. We have access to simulator runs
representing all combinations for 11 unique values of parallel mean free path
and 6 unique values of ratio {\color{red}($n_U=66$)}. The spread of parameter
values is determined by space scientists to include all values considered
reasonable: parallel mean free path ranges from 500-3000 astronomical units
(AU); ratio values are unitless, and span from 0.001 to 0.1. Due to the
inability for the model to furnish posterior samples beyond the boundary
parameters, we do not test those combinations for which at least one parameter
is on its edge (i.e., when parallel mean free path $\in \{500, 3000\}$ and/or
ratio $\in \{0.001, 0.1\}$). Removing these edge cases results in 36 remaining
test runs. For each combination, we follow the same procedure as above: select
one model parameter and set as $u^\star$, create simulated observed satellite
data, fit {\color{red}an SVEC} surrogate on training data excluding $u^\star$,
and run Algorithm \ref{alg:gibbs} to sample from the posterior $u \mid Y^F$.
Results are shown in Figure \ref{f:synth_estimates_all}.
...
Our experimental setup will be similar to that in Section
\ref{sec:synth_data}, only substituting in real data for simulated data. Space
scientists at LANL have provided us with satellite data from IBEX for the
years 2009-2011, aligning with the implementation of the computer model. The
number of records in each sky map varies across the three years from
$\sim$7-9K observations, giving us over 23K rows of field data ($X^F, Y^F$)
for use in our analysis. We execute Algorithm \ref{alg:gibbs} by fitting
{\color{red}an SVEC} GP surrogate to \textit{all} 66 computer model runs and
sample from the posterior $u \mid Y^F$ via MCMC. One difference from Section
\ref{sec:synth_data} is that field data are not exclusively from one map
(Figure \ref{f:ibex_real_ex}). Instead, ($X^F, Y^F$) is formed by stacking the
counts, exposure times, background rates, and Cartesian coordinates $x$, $y$,
$z$ from years 2009-2011 {\color{red}$\left(X^F = [X_{2009}^{F^\top}
\quad X_{2010}^{F^\top} \quad X_{2011}^{F^\top}]^\top; Y^F = [Y_{2009}^{F^T} \quad
Y_{2010}^{F^T} \quad Y_{2011}^{F^T}]^\top\right)$. Although these data come from
different years, we treat them as one field dataset. The simulator (or GP
surrogate) is evaluated in Algorithm \ref{alg:gibbs} at all $X^F$ locations,
regardless of year, to provide $\lambda(X^F)$ values for the single likelihood
calculation, $\mathcal{L}(Y^F \mid \lambda(X^F)^{(t)}, e(X^F)) = \prod_{i=1}^{n_F}
f(y_i \mid \lambda_i(X^F)^{(t)}, e_i)$. The IBEX simulator itself does not embody
any notion of time or year as a covariate or predictor, but was specifically
developed to model the heliosphere in the years 2009-2011 when the satellite
data was collected.}
...
\begin{figure}[ht!]
\centering
\includegraphics[scale=0.3,trim=0 65 30 20,clip=TRUE]{real_bayes_inv_res_2012.pdf}
\includegraphics[scale=0.3,trim=50 65 30 20,clip=TRUE]{real_bayes_inv_res_2013.pdf}
\includegraphics[scale=0.3,trim=50 65 30 20,clip=TRUE]{real_bayes_inv_res_2014.pdf}
\includegraphics[scale=0.3,trim=50 65 30 20,clip=TRUE]{real_bayes_inv_res_2015.pdf}
\includegraphics[scale=0.3,trim=50 65 30 20,clip=TRUE]{real_bayes_inv_res_2016.pdf}
\includegraphics[scale=0.3,trim=0 0 30 20,clip=TRUE]{real_bayes_inv_res_2017.pdf}
\includegraphics[scale=0.3,trim=50 0 30 20,clip=TRUE]{real_bayes_inv_res_2018.pdf}
\includegraphics[scale=0.3,trim=50 0 30 20,clip=TRUE]{real_bayes_inv_res_2019.pdf}
\includegraphics[scale=0.3,trim=50 0 30 20,clip=TRUE]{real_bayes_inv_res_2020.pdf}
\includegraphics[scale=0.3,trim=50 0 30 20,clip=TRUE]{real_bayes_inv_res_2021.pdf}
\caption{{\color{red}Posterior distributions $u \mid Y^F$ for 10 separate runs of our
Poisson Bayesian inverse problem framework. Model parameters $u_1$ and $u_2$
represent parallel mean free path and ratio, respectively. Bold, black borders
on plot insets indicate boundary values for each parameter. Field data $(X^F,
Y^F)$ for each run come from a single year between 2012-2021.}
...
\mbox{e.g.,} \quad u &\sim \mathrm{Unif}[0,1]^{p_U}.
\end{align}
...
$\delta$. Mass is concentrated just below 1, indicating nearly zero effect
from the multiplicative discrepancy. Examining the disparities closer, it
appears that spatial dependence exists in whether the simulator overestimates
or underestimates the mean surface. For instance, the area of the sky map
corresponding to the ribbon shows some underestimation. By contrast, areas
outside of the ribbon are overestimated. We believe that $\delta$ is estimated
around 1.0 because of these competing interests between different regimes in
the sky map. A constant multiplicative scale discrepancy is clearly
insufficient. Other ideas to resolve this shortcoming are listed below.
...
\subsection*{Possible Extensions \& Alternative Formulations}

{\color{red}Figure \ref{f:ibex_disc} illustrates the need for more
advancements to properly model disparity between the observed IBEX satellite
data and the corresponding simulator. A fully Bayesian framework could be
developed to enhance uncertainty quantification, perhaps with the use of a
deep Gaussian process to handle the inherent nonstationarity in the
discrepancy.}

{\color{red}Section \ref{sec:gp_approx} details several GP approximations for
modeling large-scale datasets. In particular, we review SEPIA
\citep{gattiker2020lanl}, a {\sf Python} library based on work by
\citet{higdoncalib2008}, and subsequently use it as a competitor throughout
the manuscript. SEPIA performs comparably to SVEC in terms of predictive
accuracy and uncertainty quantification. However, it suffers computationally
as the number of computer model runs increases. One intriguing extension could
involve combining \citeauthor{higdoncalib2008}'s PCA approach with Vecchia. To
our knowledge this has not been attempted, but represents a promising area of
research.}

{\color{red}Several alternative formulations and software implementations may
also be useful for addressing problems of this type. For example, {\sf GPflow}
proposes a modular system for scalable Gaussian process modeling, as described
in \citet{van2020framework}. The {\sf Python} package {\sf surmise}
\citep{surmise2025} provides related surrogate modeling functionality, in
particular as it relates to computer model calibration
\citep{chan2024constructing,surer2022uncertainty}. Variational approaches
\citep{opper2009variational,hensman2015scalable} could also be explored. These
methods offer additional modeling and computational options that may be
appropriate depending on the objectives and requirements of a particular
application.}
...
20220107DR. The authors would like to thank C.A.~Eiffer for her encouragement
and support of this paper. {\color{red}They also wish to thank the editor,
associate editor, and referees for their valuable suggestions, which greatly
improved the manuscript.} This work has been approved for public release under
LA-UR-25-30539.
...
inverse problem framework to recover the {\color{red}true, ``unknown''} model
parameters $u$ from synthetic satellite data. In particular, we made the claim
that both sky maps in Figure \ref{f:ibex_synth_ex} (top middle, top right)
could have reasonably generated the ENA counts ``observed'' in the top left
plot of that figure. This assertion was based on a purely visual assessment of
the data and the estimated underlying ENA rate. A more empirical method to
validate our claim could be done via the probability integral transform
...
the {\color{red}$n_U=66$} $u$-values and set this as $\lambda^\star$. Next, we both scale
...
\subsection{Residual Visual Analysis}
\label{app:resid_anal}

{\color{red} Figure \ref{f:ibex_sim_surr}, in the main manuscript, helped
visualize the accuracy of our proposed method (SVEC) by displaying a few
computer model evaluations side-by-side with predicted surfaces at unobserved
parameter settings. Figure \ref{f:ibex_surr_metrics} then presented
performance metrics from a hold-one-out bakeoff between SVEC and multiple
competitors. At each iteration, all methods are trained on 65 model runs of
the IBEX simulator and then asked to predict the response at the held out
model parameter. Predictions were then used to calculate the displayed RMSE
and CRPS values. To further visually demonstrate the differences between
competing methods, Figure \ref{f:surrogate_residuals} here plots residuals
(averaged over the 66 held out model runs) from each method's predicted
surfaces.}

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.29,trim=0 65 100 20,clip=TRUE]{residual_plot_SVEC_25.pdf}
\includegraphics[scale=0.29,trim=50 65 100 20,clip=TRUE]{residual_plot_SVEC_50.pdf}
\includegraphics[scale=0.29,trim=50 65 100 20,clip=TRUE]{residual_plot_SVEC_75.pdf}
\includegraphics[scale=0.29,trim=50 65 5 20,clip=TRUE]{residual_plot_laGP.pdf}
\includegraphics[scale=0.29,trim=0 0 100 10,clip=TRUE]{residual_plot_SEPIA_3.pdf}
\includegraphics[scale=0.29,trim=50 0 100 10,clip=TRUE]{residual_plot_SEPIA_4.pdf}
\includegraphics[scale=0.29,trim=50 0 100 10,clip=TRUE]{residual_plot_SEPIA_5.pdf}
\includegraphics[scale=0.29,trim=50 0 5 10,clip=TRUE]{residual_plot_deepgp.pdf}
\caption{{\color{red}Absolute, square-rooted residuals averaged over 66
hold-one-out predictions from the IBEX simulator experiment (Figure
\ref{f:ibex_surr_metrics}) for several different surrogate model techniques.
\label{f:surrogate_residuals}}}
\end{figure}

{\color{red} Note that before plotting, we first take the absolute values of
the residuals and then square root. Additionally, the range of
values for which the colors represent is clipped on the upper end. These steps
are \textit{only} taken to allow better visual comparison between methods.
Otherwise, heat maps for SVEC and SEPIA would be solid red and those for
{\sf laGP} and {\sf deepgp} solid white. }

{\color{red} It is clear that SVEC and SEPIA provide considerably better
predictions than the other methods. Additionally, the residuals from SVEC and
SEPIA predictions are similar, regardless of the neighborhood size or number
of principal components used. This hearkens back to the nearly identical
metrics observed between the two methods in Figure \ref{f:ibex_surr_metrics}.
Although it appears that these methods are missing significant structure in
the data, it is only due to these small residuals (originally on the order of
$10^{-4}$) being magnified by the modifications for visua{}l aid listed above.}

\subsection{Multivariate Performance Metrics}
\label{app:multivar_mets}

{\color{red} Section \ref{sec:emp_comp} assesses the performance of
out-of-sample predictions for the IBEX simulator using our proposed SVEC GP
surrogate and other competitors. In particular, Figure
\ref{f:ibex_surr_metrics} displays a pair of boxplots measuring each surrogate
model on RMSE and CRPS. However, RMSE and CRPS are both univariate metrics and
are not able to capture relationships between output variables (e.g. different
pixels within the same image). The response for IBEX is high-dimensional
(16,200), and therefore it is appropriate to compare using multivariate
metrics.}

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.45, trim=0 0 0 0,clip=TRUE]{ibex_surr_escores.pdf}
\includegraphics[scale=0.45, trim=0 0 0 0,clip=TRUE]{ibex_surr_linfnorm.pdf}
\caption{Multivariate performance metrics on the IBEX simulator experiment.
Smaller is better for both energy score and L-infinity distance.
\label{f:ibex_surr_multivar_metrics}}
\end{figure}

{\color{red} First, we consider \textit{energy score}
\citep{gneiting2007strictly}, a natural multivariate extension of CRPS. Energy
score incorporates two elements into the metric: 1) how far away predictions
are from the truth, and 2) how spread out the predictions are from each other.
We calculate energy score for each competitor in Figure
\ref{f:ibex_surr_metrics} using draws from the estimated predictive
distributions. Next, we measure the distance between each method's predicted
response vector $\hat{\lambda}$ and the simulator output $\lambda$ using the
infinity norm, $||\hat{\lambda} -\lambda||_\infty$. Figure
\ref{f:ibex_surr_multivar_metrics} displays the results.}

{\color{red} For the most part, the results obtained in Figure
\ref{f:ibex_surr_metrics} hold. SVEC and SEPIA are the clear winners, but are
comparable to each other in terms of performance. This similarity extends to
varying values of neighborhood size $m$ (for SVEC) and number of PCs $n_k$
(for SEPIA). It is noteworthy that {\sf laGP} performs worse (and is highly
variable) than {\sf deepgp} in terms of L-infinity distance. We believe this
could be due to the unique framework of {\sf laGP}, where each prediction is
based on a unique neighborhood of surrounding points, and therefore largely

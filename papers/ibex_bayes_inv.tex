...
We study IBEX through the lens of observational data $Y$ and a computer
implementation of a physical model $m(\cdot)$ with unknown parameters
$\boldsymbol{u}$. Given prior information about $\boldsymbol{u}$, we want to know
which $\boldsymbol{u}$'s lead to realizations of $m(\cdot)$ that could explain $Y$,
i.e., the posterior for $\boldsymbol{u} \mid Y$. This is a classic Bayesian inverse
problem \citep{kaipio2011bayesian,stuart2010inverse,knapik2011bayesian}. To be clear,
we are not directly interested in the output of our computer model, $m(\cdot)$, or
some statistical parameters $\boldsymbol{\theta}$, as shown in the top half of Figure
\ref{f:inv_v_classic}. Although $m(\cdot)=\boldsymbol{\theta}$ remains important, our
focus is on the distribution of $\boldsymbol{u} \mid Y$, or the vector of inputs
passed into the physical or mathematical model.
\begin{figure}[ht!]
\centering
...
and the approach to Bayesian Inverse Problems (bottom). Output $y$ is Gaussian
in either case, but parameters $\mu$ and $\sigma^2$ differ in where they
originate.
...
stars denote field measurements of some true, unknown process at different locations
$X$, observed with Gaussian noise. Additionally, multiple observations are made
at each location, providing replication. This is common, due to the expense of
changing configurations of a physical experiment, but it need not be the case. Light
gray lines indicate individual runs of a computer simulation $m(\cdot)$ at a sample of
different settings of parameters $\boldsymbol{u} = \begin{bmatrix} u_1 & u_2
...
To perform inference for $\boldsymbol{u} \mid Y$, we follow Bayes' rule,
$\pi(\boldsymbol{u}|Y) \propto
\mathcal{L}(Y|\boldsymbol{u})\pi(\boldsymbol{u})$, and utilize
...
assuming $Y \sim \mathcal{N}\!\left(\mu(\boldsymbol{u}),
...
$Y \sim Pois(\boldsymbol{\lambda}(\boldsymbol{u}))$, where
...
three hyperparameters: a nugget $g$, lengthscale $\theta$, and scale $\tau^2$.
All hyperparameters can be estimated using your preferred method (i.e, maximum
...
Therefore, GPs are widely used in computer experiments and Bayesian Inverse Problems.

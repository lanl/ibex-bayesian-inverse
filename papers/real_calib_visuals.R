...
field_data$nlon <- nose_center_lons(field_data$ecliptic_lon)
...
XXF <- as.matrix(XXF)
field_data$sim_rate <- predictions_scaled(fit, XXF, m=25, joint=FALSE)
...
plot(x=field_data_09$nlon, y=field_data_09$ecliptic_lat,
  col=field_data_09$disc_col, pch=16, cex=0.7, xlab="Longitude", xaxt="n",
  ylab="Latitude", xlim=xlims, ylim=ylims, cex.lab=1.1)
axis(1, at=seq(325, 25, by=-60),
  labels=c(60, 0, 300, 240, 180, 120))
...
plot(x=field_data_10$nlon, y=field_data_10$ecliptic_lat,
  col=field_data_10$disc_col, pch=16, cex=0.7, xlab="Longitude", xaxt="n",
  ylab="Latitude", xlim=xlims, ylim=ylims, cex.lab=1.1)
axis(1, at=seq(325, 25, by=-60),
  labels=c(60, 0, 300, 240, 180, 120))
fields::image.plot(zlim=c(-max_disc, max_disc), col=disc_cols,
...
plot(x=field_data_11$nlon, y=field_data_11$ecliptic_lat,
  col=field_data_11$disc_col, pch=16, cex=0.7, xlab="Longitude", xaxt="n",
  ylab="Latitude", xlim=xlims, ylim=ylims, cex.lab=1.1)
axis(1, at=seq(325, 25, by=-60),
  labels=c(60, 0, 300, 240, 180, 120))
fields::image.plot(zlim=c(-max_disc, max_disc), col=disc_cols,
...
Xmod <- Xmod[,c("x", "y", "z")]
Xfield <- unique(field_data[c("ecliptic_lon", "ecliptic_lat")])
Xfield[,c("x", "y", "z")] <- geo_to_spher_coords(Xfield$ecliptic_lat,
  Xfield$ecliptic_lon)
...
Fy  <- ppois(field_data$counts, (preds+field_data$background)*field_data$time)
Fy1 <- ppois(field_data$counts - 1, (preds+field_data$background)*field_data$time)
# Randomized PIT for discrete distributions
pit <- Fy1 + runif(length(field_data$counts)) * (Fy - Fy1)
...
for (i in 4:(length(res)-1)) {
  ...
  Xfield <- iter_field[c("ecliptic_lon", "ecliptic_lat")]
  Xfield[,c("x", "y", "z")] <- geo_to_spher_coords(Xfield$ecliptic_lat,
    Xfield$ecliptic_lon)
  Xfield <- Xfield[,c("x", "y", "z")]
  ...
  pd <- preprocess_data(md=model_data, fd=iter_field)
  fit <- fit_scaled(y=pd$ym, inputs=cbind(pd$xm, pd$um), nug=1e-4, ms=25)
  ...
  ##### Calculate PIT values
  Fy  <- ppois(iter_field$counts, (preds+iter_field$background)*iter_field$time)
  Fy1 <- ppois(iter_field$counts - 1, (preds+iter_field$background)*iter_field$time)
  # Randomized PIT for discrete distributions
  pits[[i-3]] <- Fy1 + runif(length(iter_field$counts)) * (Fy - Fy1)

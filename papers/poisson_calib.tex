...
%  Doint it over latitude and longitude
...
improve out-of-sample prediction and estimate unknown parameters when field data are
noisy, Gaussian observations and simulator data are smooth enough and small enough to
be modeled with a Gaussian process. However, not all scientific problems that require
calibration fit into that mold. For example, our motivating application consists of a
large number of counts of particles detected by a satelite and computer model output
that is extremely high-dimensional. This is only one such problem, but certaintly
many other situations fall outside the standard computer model calibration set up.
...
at a new location $x$.
\begin{equation}
Y \sim N\!\left(\mu(X), \Sigma(X)\right) \quad y^* \mid Y \sim N\!\left(\mu^*(x), \Sigma^*(x)) \\

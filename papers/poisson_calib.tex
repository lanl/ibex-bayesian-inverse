...
calibration. Section \ref{sec:scale_vec} proposes the Scaled Vecchia
...
\ref{sec:empir_results}. We finish our contribution through innovative
...
\begin{itemize}
  \item Results of 1D Poisson calibration (prediction, distribution of calibration
  parameters)
...
response. To accommodate such a situation, \citet{higdoncalib2008} employed principal
components analysis (PCA) to reduce the dimensionality of the output. Although this
was successful, we argue that had Higdon and his co-authors had access to the
computational capacity of our day, along with the proper technology leveraging such
capacity, they would have steered their implementation in a different way. We propose
our approach next.
...
We considered multiple GP surrogate implementations to model the computer simulation,
such as the {\tt R} package {\tt laGP}. However, due to its superior performance in
predictive accurary, uncertainty quantification, and execution time, we landed on the
Scaled Vecchia Approximation \citep{scaledvecchiakatzfuss2022} to model the limited
simulator output and make predictions at new locations. In fact, when discussing
further work, \citet{scaledvecchiakatzfuss2022} suggest that their method would work
as an effective tool for use in computer model calibration.
...
the data to \ref{s:ibex_sim}. The only important things to note are that there are 66
...
\begin{figure}[ht!]
\centering
\includegraphics[scale=0.3,trim=0 0 0 0,clip=TRUE]{sim_ibex_real_calib.png}

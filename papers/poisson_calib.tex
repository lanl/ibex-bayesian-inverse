...
% SECTION 2: Computer Model Calibration
%% A general review of computer model calibration
%% Review Kennedy O'Hagan framework
%% Review Dave Higdon's SEPIA?? Maybe his 2004 paper?

%%% SECTION 2.1: Generalized Computer Model Calibration
...
% SECTION 3: Gaussian Process Surrogates
%%%%% Motivate need for surrogate from previous section
...
%%% SECTION 3.1: Vecchia Approximation
%%%%%
%%%%%
%%%%%

%%% SECTION 3.2: Scaled Vecchia Approximation
%%%%%
%%%%%
%%%%%
...
posterior draws of calibration parameters $mu$ and $nu$ (right).
\label{f:calib_nosurr}}
\end{figure}
...
% Motivate need for surrogate (computer model is expensive)
In our example in \ref{sec:poisson_calib}, we had access to essentially unlimited
output from our ``computer model.'' For each proposed vector of calibration
parameters, we could generate a novel run of the simulation almost instantaneously.
However, in most applications, the computer model of interest is computationally
expensive. Due to complex and expensive operations, such as solving partial
differential equations, simulations can take hours, days, or even weeks to complete.
This limits the ability to thoroughly explore the model parameter input space, and bw
effective in prediction and parameter estimation. Therefore, we need a model (what we
call a surrogate, or emulator) fit to the simulator output to make accurate and
reliable predictions for out-of-sample values of our calibration parameters. Any
statistical model can serve as a surrogate. However, the canonical surrogate for
computer experiments and computer model calibration is the Gaussian process.
...
multivariate normal equations. Suppose we have a vector of observations $Y$ at
input locations $X$. We make the assumption that this data is normally
distributed, that is, the observed process has some mean function $(\mu(X))$
and covariance function $(\Sigma(X))$. Note that these are only functions of
the inputs $X$. However, more important than how we model observed data is
what our model predicts at a new location $x$. Eq.~(\ref{eq:mvn_eq})
articulates how to obtain mean predictions $\mu^*(x)$ and corresponding
uncertainty quantification $\Sigma^*(x)$.
\begin{align}
Y(X) \sim N\!\left(\mu(X), \Sigma(X)\right) \quad y^*(x) \mid Y(X) \sim
N\!\left(\mu^*(x), \Sigma^*(x)\right) \nonumber \\
...
common to set that to zero. In practice, the data is centered by subtracting
off the sample mean. In the zero-mean context, all the action takes place in
the covariance function. Many covariance kernels exists, such as the Gaussian,
Exponential, Matern, etc. We use the Matern kernel in our work, of which both
the Gaussian and Exponential kernels are special cases. The Matern kernel
relies on estimating three hyperparameters: a nugget $g$, a lengthscale
$\theta$, and a scale $\tau^2$. Another parameter ($\nu$) that can be
estimated in the Matern kernel controls the smoothness of the estimated
surface. But we will ignore that here and set $\nu = \frac{5}{2}$, following
common practice due to it allowing the kernel function to be expressed in a
simpler form. All hyperparameters can be estimated using maximum likelihood
estimators, method of moments estimators, or in our case, Bayesian MCMC
methods.

GPs provide a flexible, nonparametric regression tool. They were popularized
as surrogate models for computer experiments due to their ability to
interpolate between observed data points (setting nugget $g = 0$). Although
stochastic computer models are becoming more common, computer experiments are
typically deterministic. This motivates the desire for a model that
interpolates. GPs are then useful by providing predictive surfaces that give
higher estimates of variance away from observed data, and virtually no
variance at observed locations. GPs also naturally provide excellent
uncertainty quantification for out-of-sample predictions, a matter of
importance to scientists exploring these computer models, and in particular in
...
interested in. Here, we only have 40 runs at our disposal. So, we need to fit a GP on
these 40 runs and make predictions at other input combinations of interest using
\ref{eq:mvn_eq}.
...
posterior draws of the calibration parameters, shown on the right. We can see that
the MCMC does a good job of honing in on the truth. Although the posterior
distribution is not centered on the truth, it covers the truth with a 95\% credible
interval. These draws translate to predictions for what our computer model would
output, through the use of our GP surrogate. The range of evaluations indicates that
use of the surrogate allowed the MCMC to do extensive exploration of at unobserved
inputs. Most importantly, we see that our algorithm is able to combine the limited
field data with the limited simulator output to capture the true, underlying mean
process.

We can do the same thing in higher dimension, although it's harder to visualize, even
in 2D. \ref{f:calib2d_surr} shows the results of this endeavor in a 2D example.
...
Now, in each of these examples the number of computer model evaluations has been
limited ($n < 100$). As mentioned before, computational limits ensure that this is
typically the case in computer model calibration. But note that the output of our
computer model is functional, i.e. each response is a vector. Above, each simulation
was evaluate at 50 separate points. Therefore, we were really modeling 500 ($10
\times 50$) responses. Fitting a GP on 500 data points is already stretching the
limitations of a GP (decomposing a $500 \times 500$ matrix). But consider if the
functional output were of an extremely high dimension (say hundreds, or thousands).
In that case, a GP would not be able to model each individual point as a separate
response. Next we review what has been done historically with high dimensional output
in a calibration setting. Then, we will propose a modern and better-perfoming
approach.
...
In the early 2000s, \citet{higdoncalib2008} faced this challenge. As a solution, they
employed principal components analysis (PCA) to reduce the dimensionality of the
output. By reducing the the output to a small set of component weights, the dimension
of the input matrix $X$ was able to be better handled by a GP.

Although this basis representation was successful, we argue that what
\citet{higdoncalib2008} really wanted to do was represent the functional output as a
univariate response. That is, replicate the entry in the input matrix $X$ for each
...
with the novel methodology that has been proposed since the early 200s, they would
have steered their implementation in a significantly different way. We propose that
approach next and illustrate the improved performance in both predictive accuracy and
uncertainty quantification, along with a reduction in computational expense.
...
Perhaps the primary weakness of GPs is the need to decompose an $n \times n$ covariance matrix, making
it difficult to scale to large datasets. Many methods have been introduced to address
this computational burden. These include, but are not limited to, fixed-rank kriging
\citep{cressie2008fixed}, inducing points \citep{banerjee2008gaussian}, compactly
supported kernels \citep{kaufman2011efficient}, divide-and-conquer
\citep{gramacy2015local}, and nearest neighbors \citep{datta2016hierarchical,
wu2022variational}. In the context of computer model calibration,
\citet{higdoncalib2008} use a basis representation via principal components to
accommodate high-dimensional output. However, with the dramatic increase in computing
capacity over the past decade and the aforementioned advances in methodology, we
believe there are better GP approximations available to model the output of a
computer simulation. We propose the recently popularized Vecchia approximation
...
&\approx \prod_{i=1}^{n}L(Y_i \mid Y_{h(i)}) \hspace{0.3em} \mbox{ where } h(i) \subset \{1, 2, \ldots, i-1\} \hspace{0.3em} \mbox{ and } |h(i)| = m \ll n \label{eq:vecchia}
\end{align}
...
\includegraphics[scale=0.6, trim=5 15 15 50,clip=TRUE]{logit1_est.pdf}
\includegraphics[scale=0.6, trim=5 15 15 50,clip=TRUE]{logit1_post_draws.pdf}
\caption{VECCHIA vs. SEPIA vs. DEEPGP vs. LAGP TimingM aybe replace the fitting and
prediction time plots. This could look a lot like Annie's plot in her Vecchia. One
panel has each method, showing the exponential increase. And one panel has just
Vecchia on larger and larger datasets. SEPIA might do fine here. But worth a check.
\label{f:surr_time_bakeoff}}
\end{figure}
...
\includegraphics[scale=0.6, trim=5 10 15 50,clip=TRUE]{logit1_est.pdf}
\includegraphics[scale=0.6, trim=5 10 15 50,clip=TRUE]{logit1_est.pdf}
\caption{Benchmark methods on IBEX simulator data. Each method was run 66 times on
the model data with one run held out for prediction each time. Hold-one-out
cross-validation.
...
\includegraphics[scale=0.6, trim=5 15 15 50,clip=TRUE]{logit1_est.pdf}
\includegraphics[scale=0.6, trim=5 15 15 50,clip=TRUE]{logit1_post_draws.pdf}
\caption{2D POISSON CALIBRATION EXAMPLE WITH VECCHIA GP SURROGATE. Maybe include computation time panel?

...
Often computer models are comuptationally costly. Due to complex and expensive
operations, such as solving partial differential equations, simulations can take
hours, days, or even weeks to complete. For example, astrophysicists at LANL have
developed sophisticate computer models that require over 16 hours for a single run.
This severely limits the amount of simulated data that can be produced. But in order
for computer model calibration to be effective in prediction and parameter
estimation, we need to be able to explore the entire input space of the model
parameters. Therefore, we need a surrogate to make accurate and reliable
out-of-sample predictions. The canonical surrogate for computer experiments and
computer model calibration is the Gaussian process.
...
$X$. We can model this data as being normally distributed, as in
Eq.~(\ref{eq:mvn_eq}). More importantly, we are interested in what our model predicts
at a new location $x$.
\begin{align}
Y \sim N\!\left(\mu(X), \Sigma(X)\right) \quad y^* \mid Y \sim N\!\left(\mu^*(x), \Sigma^*(x)\right) \nonumber \\
\mu^*(x)=\mu(x) + \Sigma(x, X) \Sigma(X)^{-1} (Y - \mu(X)) \nonumber \\
\Sigma^*(x)=\Sigma(x, x) - \Sigma(x, X) \Sigma(X)^{-1} \Sigma(X, x) \label{eq:mvn_eq}
\end{align}
GPs rely on the estimation of three hyperparameters that can be estimated using
maximum likelihood estimators, method of moments estimators, or in our case, Bayesian
MCMC methods. GPs provide a flexible, nonparamteric regression tool. GPs were
popularized as surrogates for computer experiments due to their ability to
interpolate between observed data points. Computer experiments are typically
deterministic, although stochastic computer models are becoming increasingly common.
GPs can provide predictive surfaces that give higher estimates of variance away from
observed data, and virtually no variance at observed locations.

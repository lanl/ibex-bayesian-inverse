...
supercomputing resources, computer models have increased in complexity over
the past two decades to produce extremely high-fidelity output. Data collected
from a physical experiment can then be augmented by output from this computer
simulation. Because of this additional source of data, scientists are able to
make better predictions for the behavior of a physical process outside sampled
locations. Another advantage from using a computer model is the ability for
practitioners vary certain parameters that cannot be controlled or measured,
at least precisely, in a physical experiment (e.g. gravity). So, in addition
to helping achieve better prediction out-of-sample, the computer model output
allows statisticians to estimate the distributions of certain unknown
parameters, deepening knowledge of the physical process.

Consider observations $Y_F$ collected from a physical experiment at input
locations $X_F$, as depicted in the left panel of Figure \ref{f:toy_calib}.
Data $Y_M$ from a computer model supplement $Y_F$ as we attempt to uncover the
underlying real process $y_R$, shown in black in the center panel. Before we
move on, we need to cover one of the essential building blocks of computer
model calibration, the Gaussian process.

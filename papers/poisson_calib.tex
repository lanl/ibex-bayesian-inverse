...
In the example in \ref{sec:poisson_calib}, we had access to essentially unlimited
output from our ``computer model.'' For each set of proposed calibration parameters,
we could generate a novel run of the simulation almost instantaneously. However, in
most scientific applications, the computer model of interest is computationally
expensive. Due to complex and costly operations, such as solving partial differential
equations, simulations can take hours, days, or even weeks to complete
\citep{santner2018design}. This limits the ability to thoroughly explore the model
parameter input space in computer model calibration, thereby reducing the predictive
performance and accuracy of parameter estimates. Therefore, we need a model (what we
...
governed by some mean and covariance functions. GPs are fit to observed data and then
used to make predictions at new input locations, relying on the conditional
multivariate normal equations. For example, suppose we have a vector of observations
$Y$ at input locations $X$. We make the assumption that this data is normally
distributed with some mean function $(\mu(X))$ and covariance function $(\Sigma(X))$.
However, more important than how we model observed data is what our model predicts at
a new location $x$. Eq.~(\ref{eq:mvn_eq}) articulates how to obtain mean predictions
$\mu^*(x)$ and corresponding uncertainty quantification $\Sigma^*(x)$.
\begin{align}
...
\end{align}
Although a generic mean function is specified in Eq.~(\ref{eq:mvn_eq}), it is common
to set that to zero. In practice, the data is centered by subtracting off the sample
mean. In the zero-mean context, all the action takes place in the covariance
function. Many covariance kernels exists, such as the Gaussian, Exponential, Matern,
etc. We use the Matern kernel in our work, of which both the Gaussian and Exponential
kernels are special cases. The Matern covariance kernel relies on estimating three
hyperparameters: a nugget $g$, a lengthscale $\theta$, and a scale $\tau^2$. Another
parameter ($\nu$) that can be estimated in the Matern kernel controls the smoothness
of the estimated surface. But we will ignore that here and set $\nu = \frac{5}{2}$
(\ref{eq:matern}), following common practice due to it allowing the kernel function
to be expressed in a simpler form. All hyperparameters can be estimated using your preferred method (i.e. maximum
likelihood, method of moments). We will use Bayesian methods, specifically MCMC.
\begin{align}
...
interested in. Here, we use a Latin hypercube sample \citep{mckay1979} of size 40
over our two parameters, $\mu$ and $\nu$. So, we need to fit a GP on these 40 runs
and make predictions at other input combinations of interest using \ref{eq:mvn_eq}.
...
in 2D. But the more pressing barrier to overcome is the need for more runs in higher
dimensions. In the example above, the number of computer model evaluations was
...
computer is functional, i.e. each response is a vector. Above, each simulation was
evaluated at 20 separate points. Therefore, we were really modeling 800 ($40
...
example (\ref{f:calib2d_nosurr}). To get 10 unique values of four parameters
($\mu_1$, $\nu_1$, $\mu_2$, $\nu_2$), we'd need upwards of 10,000 simulator runs.
Combine that with a $20 \times 20$ grid over $x_1$ and $x_2$ and our response vector
will be greater than 200,000. Even if we used an LHS design of size $n=100$, the
response vector would consist of over 40,000 points. In that case, a GP would not
feasibly be able to model each individual point as a separate response. Next we
review what has been done historically with high dimensional output in a calibration
setting. Then, we will propose a modern and better-perfoming approach.

...
computer models at Los Alamos National Laboratory produced limited runs with
immensely high-dimensional output in the form of images, shape description, time
series, etc. In order to efficiently fit a surrogate model to the simulator, they
needed to perform dimension reduction. As a solution, they employed principal
components analysis (PCA). Specifically, they represented the output of a simulator
model as the sum of basis functions $\textbf{k}_i$, scaled by weights $w_i$. By
reducing the the output to a small set of component weights, the dimension of the
input matrix $X$ was able to be better handled by a GP.
\begin{align}
\textbf{y}(\textbf{x}, \textbf{u}) = \sum_{i=1}^{n_k} \textbf{k}_i \cdot w_i (\textbf{x}, \textbf{u}) \nonumber \\
w_i (\textbf{x}, \textbf{u}) \sim GP(\textbf{0}, \Sigma_{w_i}),
\label{eq:higdon_pca}
\end{align}
where $\Sigma_{w_i}$, dependent only on a limited number of combinations of
$\textbf{x}$ and $\textbf{u}$, is a small matrix. Hence, the decomposition of
$\Sigma_{w_i}$ is fast and prevents the O($n^3$) bottleneck in computation.

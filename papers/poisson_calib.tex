...
Computer model calibration is a technique employed by statisticians to pair
data collected from separate physical and computer experiments. The motivation
for such a method stems from the infeasibility to collect a sufficient (or in
some cases, any) number of physical observations. This may be due to a variety
of limitations, such as prohibitive cost or ethical consierations. Therefore,
in an effort to gain more insight, scientists develop computer models that
attempt to represent that physical process. As a result of increased access to
supercomputing resources, computer models have increased in complexity
(cite??) over the past two decades to produce extremely high-fidelity output.
Data collected from a physical experiment can then be augmented by output from
this computer simulation. Scientists are able to make better predictions for
the behavior of a physical process outside sample locations. Computer models
also allow practitioners to vary certain parameters that cannot be controlled
in a physical experiment (e.g. gravity). So, in addition to helping achieve
better prediction out-of-sample, the computer model output allows
statisticians to estimate the distributions of certain unknown parameters,
deepening knowledge of the physical process.

% SECTION 2.1 - KOH Review:
\subsection{Kennedy \& O'Hagan Computer Model Calibration}
\label{sec:koh_review}

%% Review Kennedy O'Hagan framework
In 2001, \citet{kennedyohagan2001} introduced what has become the canonical
framework for computer model calibration. In this framework, field data and
computer simulation output are modeled jointly as following a multivariate
normal distribution. \citet{kennedyohagan2001} make two key suggestions: 1)
Computer models, despite improvements in recent years, are still inherently
biased. It's virtually impossible to exactly replicate a physical process
through simulation. 2) Although often easier to conduct than physical
experiments, computer models are still expensive in terms of computation,
...
%%% Motivate need for surrogate from previous section. Expensive computer model and field data.
In Section \ref{sec:poisson_calib}, both toy examples had access to essentially
unlimited output from our ``computer model.'' For each set of proposed computer model
parameters, we could generate a novel run of the simulation almost instantaneously.
This allowed us to retrieve exact evaluations at any point in the entire input space.
However, in most scientific applications, the computer model of interest is
computationally expensive. Due to complex and costly operations, such as solving
partial differential equations, simulations can take hours, days, or even weeks to
complete \citep{santner2018design}. This limits the ability to thoroughly explore the
...
statistical model can serve as a surrogate, ranging from linear regression to complex
neural networks \citep{??}. However, the canonical surrogate for computer experiments and
computer model calibration is the Gaussian process \citep{gramacy2020surrogates}.
...
multivariate normal equations, also known as the \textit{kriging} equations
\citep{??}. For example, suppose we have a vector of observations $Y$ at input
locations $X$. $Y$ is an $n \times 1$ vector and $X$ is an $n \times p$ matrix. We
make the assumption that $Y$ is normally distributed with some mean function
$(\mu(X))$ and covariance function $(\Sigma(X))$. However, more important than how we
model observed data is what our model predicts at a new location $x$.
Eq.~(\ref{eq:mvn_eq}) articulates how to obtain mean predictions $\mu^*(x)$ and
corresponding uncertainty quantification $\Sigma^*(x)$.
\begin{align}
...
to set $\mu(x)=0$. In practice, $Y$ is centered by subtracting off the sample mean. In
the zero-mean context, all the action takes place in the covariance function. Many
covariance kernels exists, such as the Gaussian, Exponential, Matern, etc \citep{??}.
We use the Matern kernel in our work, of which both the Gaussian and Exponential
kernels are special cases. Covariance kernels typically depend on three
hyperparameters: a nugget $g$, a lengthscale $\theta$, and a scale $\tau^2$. In the
Matern kernel, an additional parameter ($\nu$) exists, which controls the smoothness
of the estimated surface. But we will not estimate the smoothness parameter here and
instead set $\nu = \frac{5}{2}$ (\ref{eq:matern}), following common practice due to
it allowing the kernel function to be expressed in a simpler form. All
...
\label{eq:matern}
\end{align}
GPs provide a flexible, nonparametric regression tool. They were popularized as
...
are becoming more common, computer experiments are often deterministic. This
motivates the desire for a model that interpolates. GPs are then useful by providing
predictive surfaces that give higher estimates of variance away from observed data,
and virtually no variance at observed locations. GPs also naturally provide excellent
uncertainty quantification for out-of-sample predictions, a matter of importance to
scientists exploring these computer models. In computer model calibration, we use a
GP to model the simulator of interest.

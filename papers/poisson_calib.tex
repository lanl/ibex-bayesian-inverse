...
%% Scientists need to solve the inverse problem
%% Often there is a physics-based, mathematical model that represents the physical experiment
%% Noisy response in the field makes it harder to infer the true parameter values of the mathematical model
%% We don't just want a point estimate, but full uncertainty quantification of the parameters
%% We use bayesian inverse problems to estimate parameters, but can also be used for inputs
%% Often limited data in field <- not sure if this is a Bayesian inverse problem problem, but it fits our situation
%% Reference that computer model calibration does this exact thing, but extends to a discrepancy
%% Show prior/posterior for parameters in equation form
%% Analytical solution is not possible
%% Must resort to MCMC
%% Classical methods might use the mathematical model and just run as many different parameter combinations as possible to fit the data <- maximum likelihood
%% But we have an expensive computer model

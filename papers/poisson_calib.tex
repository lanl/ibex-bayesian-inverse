...
Many computer models to generate sky maps exist (???). In this paper we focus
on two. One simulation to create variations of the GDF and one that offers and
explanation of how the ribbon is produced. \citet{zirnsteinGDFsims2015}
developed a model to replicate the generation of ENAs at the GDF based on two
parameters, param1 and param2. The 
...
for space scientists to perform analysis, especially separating signal from
the noise. The IBEX Science Operations Center (ISOC) perform some aggregation
of the raw data to produces maps, but these still retain a significant amount
of noise. To remedy this problem \citet{osthustheseus2023} propose a method
called Theseus, which takes the raw data and generates a much smoother
representation of the surface, using a series of Generative Additive Models
(GAM) and Projection Pursuit Regression. Once those Theseus maps are created,
they are separated into GDF-only and ribbon-only maps
\citep{beesleyribbonsep2023}. With the source of ENAs separated into two
distinct maps, space scientists perform analysis and conduct research
exclusively for either the GDF or ribbon with the respective map. Both Theseus
and ribbon separation rely on many assumptions about the data and thus depart
from the data generation process itself.

% Introduce a charicature of the IBEX problem

%%% SEPIA: Dave Higdon's LANL - we're going back to what Dave wanted to do, but couldn't at the time with computational limits
The canonical framework for statistical computer model calibration was proposed
by \citet{kennedyohagan2001}. However, KOH relies on modeling the simulation
runs and field data jointly via a multivariate normal representation. The field
data collected by IBEX comes in Poisson-distributed as counts, preventing use
of KOH. \citet{higdoncalib2004} expanded on the KOH framework to include the
scenario where simulation evaluations are cheap and can be retrieved at will.
Unfortunately, as noted above, an individual run of the IBEX simulation can
take up to 16 hours to complete, precluding the assumption of an infinite
supply of model data. Additionally, the output from the IBEX simulation is
extremely high dimensional. Each run of the simulation produces a vector of
16,2000 ENA rates that form a sky map. The computational burden of inverting a
large covariance matrix in a GP surrograte necessitates approximation.
\citet{higdoncalib2008} rely on a basis representation via principal components
to reduce the dimension of the data. Use of a basis representation was
necessary due to a lack of computational ability, something that has changed in
the past two decades.

Previous work has been done to generalize calibration to non-Gaussian
responses \citep{grosskopfcountcalib2020}. However, applications were
restricted to one-dimensional input spaces. Higher-dimensions brings multiple
challenges, including the need to scale up to larger datasets. No
approximation method was necessary in their implementation. Our use of the
Scaled Vecchia approximation was actually suggested by
\citet{scaledvecchiakatzfuss2022} in the discussion, noting its ability to
scale well. Our proposed framework allows for both large scale datasets and
the ability to extend to any type of response.

Toward that end, the paper is outlined as follows. We review GPs as surrogates
in computer model calibration and the work that has been done to apply
calibration to a wide array of problems in Section \ref{sec:review}. Section
\ref{sec:poisson_calib} introduces an McMC framework as an extension of KOH to
account for non-Gaussian data in the context of calibration. We propose the use
of the Scaled Vecchia Approximation \citep{scaledvecchiakatzfuss2022} to
account for large-scale data in \ref{sec:scale_vec}. We provide empirical
results for small and large problems in Section \ref{sec:ibex_sim}. We finish
our contribution through application to the real IBEX satellite data in Section
\ref{sec:ibex_real}. Section \ref{sec:discuss} follows up and concludes our
paper with a discussion. Code reproducing all examples is provided at
\url{https://github.com.org/lanl/IBEX_Calibration}.

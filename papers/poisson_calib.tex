...
In our example above, we had access to essentially unlimited output from our
``computer model.'' For each proposed vector of calibration parameters, we
could generate a novel run of the simulation almost instantaneously. However,
in many, or perhaps most, applications, the computer model of interest is
computationally expensive. Due to complex and expensive operations, such as
solving partial differential equations, simulations can take hours, days, or
even weeks to complete. But in order for computer model calibration to be
effective in prediction and parameter estimation, we need to be able to
explore more of the model parameter input space than the limited output from
an expensive simulation. Therefore, we need a model (what we call a surrogate
or emulator) fit to the simulated data to make accurate and reliable
predictions for out-of-sample values of our calibration parameters. Any
statistical model can serve as a surrogate. Even machine learning models can
potentially serve this purpose. However, the canonical surrogate for computer
experiments and computer model calibration is the Gaussian process.
...
Now, let's go back to our example in Section ??. There, we were given a
certain amount of observed field data and access to a computer model that
could quickly spit out results for any unique combination of parameters we
were interested in. Here, we only have 10 runs at our disposal. So, we need to
fit a GP on these 10 runs and make predictions at other input combinations
of interest using that model.
...
\includegraphics[scale=0.6, trim=5 15 15 50,clip=TRUE]{logit1_est.pdf}
\includegraphics[scale=0.6, trim=5 15 15 50,clip=TRUE]{logit1_post_draws.pdf}
\caption{1D POISSON CALIBRATION EXAMPLE WITH GP SURROGATE.
...
\begin{figure}[ht!]
\centering
\includegraphics[scale=0.6, trim=5 10 15 50,clip=TRUE]{surr_fittimes.pdf}
\includegraphics[scale=0.6, trim=5 10 15 50,clip=TRUE]{surr_predtimes.pdf}
\includegraphics[scale=0.6, trim=5 10 15 50,clip=TRUE]{surr_rmses.pdf}
\includegraphics[scale=0.6, trim=5 10 15 50,clip=TRUE]{surr_crps.pdf}

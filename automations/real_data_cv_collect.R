...
us <- array(NA, dim=c(nrow(res$mcmc_res$u), ncol(res$mcmc_res$u), length(files)))
logscls <- matrix(NA, nrow=nrow(res$mcmc_res$logscl), ncol=length(files))
...
for (i in 1:length(files)) {
  ...
  us[,,i] <- res$mcmc_res$u
  logscls[,i] <- res$mcmc_res$logscl
}

metrics <- list(crps_pmfp=crps_pmfp, crps_ratio=crps_ratio, crps_grid=crps_grid,
  pmfp_grid=pmfp_grid, ratio_grid=ratio_grid, grid=grid, post_means=post_means,
  us=us, logscls=logscls)
